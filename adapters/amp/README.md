...
`onibi agent inspect --agent amp` reports all five generated and observed plugin hooks, backup, source/hash drift, and reload instructions. Hermetic fixtures cover approve, deny, expiry, edit, daemon-unavailable allow, timeout allow, plugin reload, and distinct provider thread IDs. `onibi agent status --json` reports Amp's non-certified contract v1; its omitted `minimum_provider_version` means no provider floor has been established.
