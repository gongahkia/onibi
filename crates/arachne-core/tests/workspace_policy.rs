...
fn grpc_codegen_uses_the_pinned_vendored_protobuf_compiler() {
    ...
    for package in ["arachne-daemon-api", "arachne-relay-api"] {
...
fn api_version_policy_limits_publishable_rust_surface() {
    ...
    let sdk_manifest = fs::read_to_string(workspace_file("crates/arachne-sdk/Cargo.toml"))
...
fn ci_exercises_the_tor_maildrop_topology() {
    ...
    assert!(
        workflow.contains("cargo test -p arachne-daemon --test tor_maildrop_topology --locked")
    );
}
...
fn ci_runs_end_to_end_tests_on_every_supported_platform() {
    ...
    assert!(workflow.contains(
        "cargo test -p arachne-daemon --test direct_two_node --test lan_direct_transport --test local_mesh_two_node --test wifi_direct_transport --test wifi_hotspot_transport --locked"
    ));
}
...
fn ci_verifies_reproducible_release_artifacts() {
    ...
    assert!(workflow.contains("CARGO_INCREMENTAL=0 CARGO_TARGET_DIR=target/repro-a cargo build --release -p arachne-cli --locked"));
    assert!(workflow.contains("CARGO_INCREMENTAL=0 CARGO_TARGET_DIR=target/repro-b cargo build --release -p arachne-cli --locked"));
    assert!(workflow.contains("cmp target/repro-a/release/arachne target/repro-b/release/arachne"));
    assert!(
        workflow.contains(
