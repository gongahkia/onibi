...
fn decode_prekey(encoded: &[u8]) -> Result<X25519PrekeyPublicKey, PrekeyBundleError> {
    if encoded.len() != yeokcham_core::X25519_KEY_BYTES {
        return Err(PrekeyBundleError::InvalidPrekey);
    }
    let mut bytes = [0; yeokcham_core::X25519_KEY_BYTES];
    bytes.copy_from_slice(encoded);
...
mod tests {
    ...
    use yeokcham_core::{IdentityKeypair, OneTimePrekeyId, X25519Prekey};
