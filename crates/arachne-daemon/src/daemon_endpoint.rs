...
pub const DEFAULT_DAEMON_UNIX_SOCKET_NAME: &str = "arachne-daemon.sock";
pub const DEFAULT_DAEMON_WINDOWS_NAMED_PIPE_NAME: &str = "arachne-daemon";
...
mod tests {
    ...
    fn config_path() -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "arachne-daemon-endpoint-{}-{number}.conf",
            std::process::id()
        ))
    }
    ...
    fn parses_versioned_unix_and_windows_local_endpoints() {
        ...
        let pipe = DaemonEndpointConfig::parse(
            "endpoint_version = 1\nendpoint_kind = \"windows_named_pipe\"\nendpoint_name = \"arachne-daemon\"\n",
        )
        .unwrap();
        assert_eq!(
            pipe.endpoint(),
            &DaemonEndpoint::WindowsNamedPipe("arachne-daemon".to_owned())
        );
