...
mod tests {
    ...
    use arachne_protocol::WifiGroupCredential;
    ...
    fn uses_volatile_rsn_hotspot_and_peer_bound_p2p_settings() {
        ...
        let owner = hotspot_settings("wlan0", "arachne", &credential, true).unwrap();
        let client = hotspot_settings("wlan0", "arachne", &credential, false).unwrap();
        ...
        let p2p = p2p_settings("wlan0", "arachne", &peer).unwrap();
        assert_eq!(
