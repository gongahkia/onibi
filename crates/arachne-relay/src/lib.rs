...
mod tests {
    ...
    use arachne_core::{KeystoreEntryName, KeystoreSecret, OsKeystore, RelaySigningKeypair};
    use arachne_protocol::{
    ...
    fn config_path() -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "arachne-relay-config-{}-{number}.conf",
            std::process::id()
        ))
    }
...
database_path = "/var/lib/arachne/relay.sqlite"
...
"#
    ...
    fn validates_self_hosted_relay_configuration() {
        ...
        let config = SelfHostedRelayConfig::new(
            ...
            "/var/lib/arachne/relay.sqlite".into(),
            quota,
            retention,
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            std::path::Path::new("/var/lib/arachne/relay.sqlite")
        );
