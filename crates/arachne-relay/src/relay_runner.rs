...
use arachne_core::{RELAY_IDENTITY_SERIALIZED_BYTES, RelayPublicKey, RelaySigningKeypair};
use zeroize::Zeroize;
...
pub enum RelayIdentityFileError {
    ...
    Invalid(#[source] arachne_core::RelayIdentitySerializationError),
}
...
pub enum RelayIdentityFileGenerationError {
    ...
    Randomness(#[source] arachne_core::RelayIdentityKeyError),
...
mod tests {
    ...
    fn temporary_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "arachne-relay-runner-{name}-{}-{number}",
            std::process::id()
        ))
    }
