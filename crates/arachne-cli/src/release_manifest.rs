...
const SIGNING_DOMAIN: &[u8] = b"arachne/v1/release-artifact-manifest";
...
mod tests {
    ...
    fn verifies_regular_artifacts_and_rejects_tampering() {
        let root =
            std::env::temp_dir().join(format!("arachne-release-manifest-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir(&root).unwrap();
        let path = root.join("arachne");
    ...
    fn rejects_unsafe_artifact_names() {
        for name in ["../arachne", "arachne\\x", "Arachne", "yeok/cham"] {
            assert!(!is_canonical_artifact_name(name));
        }
        assert!(is_canonical_artifact_name("arachne-linux-x86_64"));
        assert!(ReleaseArtifact::from_path(&PathBuf::from("../arachne")).is_err());
    }
}
