...
use yeokcham_daemon::{
    DirectConnectionAttempts, DirectTransport, LocalTransport, WifiDirectTransport,
};
...
async fn authenticated_wifi_direct_adapters_conform_over_direct_quic() {
    ...
    let node_a_adapter = WifiDirectTransport::new(node_a_connection);
    let node_b_adapter = WifiDirectTransport::new(node_b_connection);
    assert!(
        node_a_adapter
            .send_frame(
                &WireEnvelope {
