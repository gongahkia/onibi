...
use yeokcham_daemon::{
    DirectConnectionAttempts, DirectTransport, LocalTransport, WifiHotspotTransport,
};
use yeokcham_protocol::{
    DirectProfileConfig, EnvelopeKind, LocalMeshTransportKind, ProtocolVersion, WireEnvelope,
    WireLimits,
};
...
async fn authenticated_wifi_hotspot_adapters_conform_over_direct_quic() {
    ...
    let node_a_adapter = WifiHotspotTransport::new(node_a_connection);
    let node_b_adapter = WifiHotspotTransport::new(node_b_connection);
    assert!(
        node_a_adapter
            .send_frame(
                &WireEnvelope {
