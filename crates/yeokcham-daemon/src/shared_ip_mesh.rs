...
pub const SHARED_IP_MESH_TLS_KEY_ENTRY: &str = "yeokcham_shared_ip_mesh_tls_v1";
pub const SHARED_IP_MESH_TLS_PIN_BYTES: usize = 32;
pub const SHARED_IP_MESH_SERVER_NAME: &str = "yeokcham.local";
...
pub enum SharedIpMeshError {
    ...
    Peer(#[source] yeokcham_protocol::LocalMeshPeerError),
    #[error("shared-IP mesh direct profile is invalid: {0}")]
    Profile(#[source] yeokcham_protocol::DirectProfileConfigError),
...
mod tests {
    ...
    fn config_round_trips_canonically() {
        let path = PathBuf::from("/var/lib/yeokcham/alice");
        ...
        .unwrap();
        let identity = yeokcham_core::IdentityKeypair::generate()
            .unwrap()
            .public_key();
        config
            ...
            .unwrap();
        let encoded = format!(
            "# shared-IP mesh\nmesh_version = {SHARED_IP_MESH_CONFIG_VERSION}\nstate_directory = \"/var/lib/yeokcham/alice\"\ntransport = \"lan\"\nlisten_endpoint = \"192.0.2.10:4242\"\npeer = \"{}:{}\"\n",
    ...
    fn config_rejects_unsupported_transport_and_duplicate_peer() {
        ...
        let peer = SharedIpMeshPeer::new(
            yeokcham_core::IdentityKeypair::generate()
                .unwrap()
                .public_key(),
            SharedIpMeshCertificatePin::from_certificate_der(b"peer certificate"),
        );
