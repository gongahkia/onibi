...
import { AgentSdkCodeGenerator, GeneratedNodeBuildLoop, LocalCodegenArtifactStore, OpenAiCodeGenerator, OpenWeightCodeGenerator, assertSafeArtifactPath, buildTbom, checksumArtifactContent, createArtifactManifest, createAgentSdkGeneratedNodeRoleRunners, createOpenAiGeneratedNodeRoleRunners, createOpenWeightGeneratedNodeRoleRunners, createGeneratedArtifact, resolveAzureOpenAiResponsesConfig, synthesizeWorkflowFromTrajectory } from "@kelpclaw/codegen";
import { getSkill, listSkills, loadPromotedSkills, registerPromotedSkill } from "@kelpclaw/skill-registry";
import { createDefaultLiveAdapters, createDefaultMockAdapters, createMcpAdapter, createOpenApiAdapter, importMcpConnector, importOpenApiConnector, testMcpConnector, testOpenApiConnector } from "@kelpclaw/adapters";
import { CronExpressionParser } from "cron-parser";
import { AdapterBackedNodeRunner, MockNodeRunner, ProductionNodeRunner, SecretStoreResolver, compileWorkflowDag, evaluateDraftWorkflow, executeCompiledDag } from "@kelpclaw/nanoclaw";
import { WorkflowValidationError, createWorkflowGraphDiff, createWorkflowPlannerFeedback, createWorkflowRuntime, createWorkflowSpecDiff, gmailReceiptsToSheetsWorkflowFixture, redactJsonRecord, redactSecretString, stableJsonStringify, validateWorkflowSpec } from "@kelpclaw/workflow-spec";
import { createDeterministicPlannerBackend, createPlannerBackendFromEnv, planMockWorkflowDraft, planWorkflowDraft, repromptWorkflow } from "./planner.js";
...
import { InMemorySecretStore, SqliteSecretStore, consumeOAuthState, createOAuthState, secretReadiness } from "./secrets.js";
import { InMemoryAgentRunStore, SqliteAgentRunStore, createAgentRunAuditAnchor } from "./agent-run-store.js";
import { registerAgentRunRoutes } from "./agent-run-routes.js";
import { attachAuthPrincipal, authPrincipalForRequest, createRoleToken, createApiAuthContext, inspectApiToken, isApiRole, principalHasRole } from "./auth.js";
...
export function createConfiguredWorkflowStore() {
    if (process.env.KELPCLAW_WORKFLOW_STORE === "memory") {
        return new InMemoryWorkflowStore();
    }
    return new SqliteWorkflowStore({
        databasePath: process.env.KELPCLAW_WORKFLOW_DB ?? join(process.cwd(), ".kelpclaw", "workflow.sqlite")
    });
}
export function createConfiguredSecretStore() {
    const store = process.env.KELPCLAW_SECRET_STORE === "memory"
        ? new InMemorySecretStore()
        : new SqliteSecretStore({
            databasePath: process.env.KELPCLAW_SECRET_DB ??
...
export function createConfiguredAgentRunStore() {
    if (process.env.KELPCLAW_AGENT_RUN_STORE === "memory") {
        return new InMemoryAgentRunStore();
    }
    return new SqliteAgentRunStore({
        databasePath: process.env.KELPCLAW_AGENT_RUN_DB ??
            process.env.KELPCLAW_WORKFLOW_DB ??
            join(process.cwd(), ".kelpclaw", "workflow.sqlite")
    });
}
function createLiveGeneratedNodeProvidersFromEnv() {
...
function codegenProviderFromEnv() {
    const provider = process.env.KELPCLAW_CODEGEN_PROVIDER ?? process.env.KELPCLAW_PLANNER_PROVIDER ?? "anthropic";
...
function codegenModelForProvider(provider) {
    switch (provider) {
        case "anthropic":
...
function seedConfiguredSecrets(secretStore) {
...
export function buildApiApp(options = {}) {
    ...
    const shouldRehydratePromotedSkills = options.rehydratePromotedSkills ?? process.env.NODE_ENV !== "test";
    ...
    jobWorker.register("run.workflow", async (job, signal) => executeWorkflowRunJob({
        store,
        secretStore,
        artifactStore,
        runner,
    ...
    cleanupWorker.start();
    const adminToken = options.adminToken === undefined ? process.env.KELPCLAW_ADMIN_TOKEN : options.adminToken;
    ...
    app.post("/api/auth/role-tokens", { preHandler: auth.requireRole("admin") }, async (request, reply) => {
        ...
        const signingSecret = options.authSigningSecret === undefined
    ...
    app.post("/api/auth/role-tokens/inspect", { preHandler: auth.requireRole("admin") }, async (request, reply) => {
    ...
    app.post("/api/agent-runs/:id/audit/anchor", { preHandler: auth.requireRole("admin") }, async (request, reply) => {
    ...
    app.post("/api/agent-runs/:id/promote", { preHandler: auth.requireRole("reviewer") }, async (request, reply) => {
        const run = agentRunStore.getRun(request.params.id);
        if (!run) {
            return reply.code(404).send({
                ok: false,
        ...
        const loadedSkill = JSON.parse(await artifactStore.readArtifact(storedSkillArtifact.ref));
    ...
    app.get("/api/agent-runs/:id/tbom", { preHandler: auth.requireRole("auditor") }, async (request, reply) => {
        const run = agentRunStore.getRun(request.params.id);
        if (!run) {
            return reply.code(404).send({
                ok: false,
    ...
    app.get("/api/skills", { preHandler: auth.requireRole("auditor") }, async (request) => {
        ...
        const skills = listSkills().filter((skill) => {
            ...
            const promptMatches = !query.prompt ||
    ...
    app.post("/api/skills/:skillId/invoke", { preHandler: auth.requireRole("operator") }, async (request, reply) => {
    ...
    app.post("/api/router/evaluate", async (request, reply) => {
    ...
    app.post("/api/jobs/:jobId/cancel", async (request, reply) => {
        ...
        const withEvent = jobSupervisor.cancelJob(job.id, request.body.reason ?? "Cancelled by API request.", now);
        return {
            ok: true,
            job: withEvent
        };
    ...
    app.delete("/api/secrets/:name", async (request) => ({
        ok: true,
    ...
    app.get("/api/integrations/google/callback", async (request, reply) => {
        ...
        secretStore.putSecret("google.oauth.default", JSON.stringify({
    ...
    app.post("/api/connectors/openapi/import", async (request, reply) => {
        try {
            const id = request.body.id ??
                `connector.openapi.${slugify(request.body.name ?? request.body.sourceUrl ?? "openapi")}.${Date.now()}`;
            const connector = store.saveConnector(await importOpenApiConnector({
                id,
    ...
    app.post("/api/connectors/mcp", async (request, reply) => {
        try {
            const id = request.body.id ??
                `connector.mcp.${slugify(request.body.name ?? request.body.endpointUrl)}.${Date.now()}`;
            const connector = store.saveConnector(await importMcpConnector({
                id,
                name: request.body.name,
    ...
    app.delete("/api/connectors/:connectorId", async (request) => {
        const connector = store.getConnector(request.params.connectorId);
    ...
    app.post("/api/connectors/:connectorId/test", async (request, reply) => {
        const connector = store.getConnector(request.params.connectorId);
        if (!connector) {
            return reply.code(404).send({
                ok: false,
        ...
        try {
            const tested = store.saveConnector(connector.kind === "mcp"
    ...
    app.post("/api/plans/mock", async (request) => {
    ...
    app.post("/api/workflows/plan", async (request, reply) => {
        const correlationId = correlationIdForRequest(request);
        ...
        if (clarification) {
            updateRequestJob(store, request, "succeeded", "Planning paused for clarification.", {
                route: route.route,
        ...
        try {
            ...
            const routedPlanner = route.requiredModel.mode === "none"
            ...
            updateRequestJob(store, request, cancelled ? "cancelled" : "failed", cancelled ? error.message : "Workflow planning failed.", {
                route: route.route
            }, jobSupervisor);
            return reply.code(503).send({
                ok: false,
        ...
        persistCodegenArtifactManifests(store, draftRevision.workflow, draftRevision.id, draftRevision.createdAt);
    ...
    app.post("/api/workflows/:id/branches", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
        ...
        const draftRevision = store.saveDraftRevision(sourceDraft.workflow, sourceDraft.validation, "branch-fork", {
            branchId,
            force: true,
            preserveRevision: true,
    ...
    app.get("/api/workflows/:id/branches", async (request, reply) => {
    ...
    app.get("/api/workflows/:id/branches/:branchId", async (request, reply) => {
    ...
    app.patch("/api/workflows/:id/branches/:branchId", async (request, reply) => {
        const branch = store.getBranch(request.params.branchId);
        if (!branch || branch.workflowId !== request.params.id) {
            return reply.code(404).send({
                ok: false,
        ...
        if (nextStatus === "active" &&
            store
                ...
                .some((candidate) => candidate.id !== branch.id &&
    ...
    app.get("/api/workflows/:id/branches/:branchId/reuse-candidates", async (request, reply) => {
        const branch = store.getBranch(request.params.branchId);
        ...
        const decisions = computeGeneratedModuleReuseDecisions(store, request.params.id, branch.id, headDraft.workflow).map((decision) => store.saveGeneratedModuleReuseDecision(decision));
        return {
            ok: true,
            decisions
        };
    });
    app.post("/api/workflows/:id/branches/:branchId/plan", async (request, reply) => {
        const branch = store.getBranch(request.params.branchId);
        const headDraft = branch ? store.getDraftRevision(branch.headDraftRevisionId) : undefined;
        if (!branch || branch.workflowId !== request.params.id || !headDraft) {
            return reply.code(404).send({
    ...
    app.post("/api/workflows/:id/branches/:branchId/reprompt-node", async (request, reply) => {
        const branch = store.getBranch(request.params.branchId);
        const headDraft = branch ? store.getDraftRevision(branch.headDraftRevisionId) : undefined;
    ...
    app.post("/api/workflows/:id/branches/:branchId/accept-plan", async (request, reply) => {
        const branch = store.getBranch(request.params.branchId);
        const headDraft = branch ? store.getDraftRevision(branch.headDraftRevisionId) : undefined;
        if (!branch || branch.workflowId !== request.params.id || !headDraft) {
            return reply.code(404).send({
        ...
        const draftRevision = store.saveDraftRevision(validation.workflow, validation, "plan-accepted", {
            branchId: branch.id,
            force: true,
            preserveRevision: true,
            parentDraftRevisionId: headDraft.id
        ...
        recordAudit(store, {
            ...
            revisionId: draftRevision.id,
            correlationId: correlationIdForRequest(request),
            summary: "Accepted branch workflow plan shape before implementation; production approval still requires implementation and draft evaluation.",
            secretRefs: collectSecretRefs(draftRevision.workflow),
            metadata: {
    ...
    app.post("/api/workflows/:id/branches/:sourceBranchId/merge-preview", async (request, reply) => {
        ...
        if (sourceBranch?.status === "archived") {
            return reply.code(409).send(archivedBranchApiError(sourceBranch));
        }
        if (targetBranch?.status === "archived") {
            return reply.code(409).send(archivedBranchApiError(targetBranch));
        }
        const preview = createBranchMergePreview(store, request.params.id, request.params.sourceBranchId, {
    ...
    app.post("/api/workflows/:id/branches/:sourceBranchId/merge", async (request, reply) => {
        const sourceBranch = store.getBranch(request.params.sourceBranchId);
        ...
        if (requestedTargetBranch?.status === "archived") {
            return reply.code(409).send(archivedBranchApiError(requestedTargetBranch));
        }
        const preview = createBranchMergePreview(store, request.params.id, request.params.sourceBranchId, {
            targetBranchId: request.body.targetBranchId,
            mode: request.body.mode ?? "merge",
        ...
        const merge = store.saveBranchMerge({
            ...preview,
            status: "applied",
        ...
        const graphDiff = store.saveGraphDiff(createWorkflowGraphDiff({
    ...
    app.post("/api/workflows/:id/validate", async (request, reply) => {
        if (request.body.workflow.id !== request.params.id) {
            return reply.code(409).send({
                ok: false,
                validation: {
    ...
    app.post("/api/workflows/:id/accept-plan", async (request, reply) => {
        if (request.body.workflow.id !== request.params.id) {
            return reply.code(409).send({
                ok: false,
                error: "WORKFLOW_ID_MISMATCH",
        ...
        recordAudit(store, {
            ...
            revisionId: draftRevision.id,
            correlationId: correlationIdForRequest(request),
            summary: "Accepted workflow plan shape before implementation; production approval still requires implementation and draft evaluation.",
            secretRefs: collectSecretRefs(draftRevision.workflow),
            metadata: {
    ...
    app.post("/api/workflows/:id/feedback", async (request, reply) => {
        if (request.body.baseWorkflow.id !== request.params.id ||
        ...
        recordAudit(store, {
            ...
            actor: "router",
            workflowId: request.params.id,
            revisionId: store.getLatestDraftRevision(request.params.id)?.id ??
        ...
        const feedback = store.savePlannerFeedback(createWorkflowPlannerFeedback({
    ...
    app.post("/api/workflows/:id/feedback/:feedbackId/suggestions/:suggestionId/decision", async (request, reply) => {
        ...
        const suggestion = feedback.suggestions.find((candidate) => candidate.id === request.params.suggestionId);
        if (!suggestion) {
            return reply.code(404).send({
                ok: false,
        ...
        const updatedFeedback = store.savePlannerFeedback({
            ...feedback,
            suggestions: feedback.suggestions.map((candidate) => candidate.id === suggestion.id
                ? {
                    ...candidate,
    ...
    app.post("/api/workflows/:id/evaluate-draft", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
        ...
        try {
            ...
            evaluation = store.saveDraftEvaluation(await evaluateDraftWorkflow(workflow, {
                draftRevisionId: draftRevision?.id,
                branchId: branch?.id,
                jobId: requestJobId(request),
                codegenArtifactStore: artifactStore,
                runGeneratedNodesInDocker: request.body.mockOnly === true ? false : process.env.NANOCLAW_DRAFT_DOCKER !== "0",
        ...
        updateRequestJob(store, request, evaluation.status === "passed" ? "succeeded" : "failed", "Draft evaluation completed.", {
            evaluationId: evaluation.id,
            readyForApproval: evaluation.readyForApproval
        }, jobSupervisor);
        return {
    ...
    app.post("/api/workflows/:id/approvals", async (request, reply) => {
        try {
            const issues = await validateApprovalReadiness(store, store.requireWorkflow(request.params.id).workflow, artifactStore);
            if (issues.length > 0) {
                return reply.code(409).send({
            ...
            const handoffArtifact = persistWorkflowHandoffArtifact(store, approvedRevision, correlationId);
            recordAudit(store, {
    ...
    app.post("/api/workflows/:id/reprompt-node", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
    ...
    app.post("/api/workflows/:id/approve", async (request, reply) => {
        const branch = request.body.branchId ? store.getBranch(request.body.branchId) : undefined;
        if (request.body.branchId && (!branch || branch.workflowId !== request.params.id)) {
            return reply.code(404).send({
                ok: false,
        ...
        try {
            const codegenIssues = await validateApprovalReadiness(store, validation.workflow, artifactStore, request.body.branchId);
            ...
            const approvedRevision = store.approveWorkflow(request.params.id, request.body.approvedBy, validation.workflow, request.body.branchId);
            const correlationId = correlationIdForRequest(request);
            const handoffArtifact = persistWorkflowHandoffArtifact(store, approvedRevision, correlationId);
            recordAudit(store, {
                action: "workflow.approved",
    ...
    app.post("/api/workflows/:id/codegen/:nodeId/build", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
        ...
        const projectedCostUsd = Math.min(request.body.maxModelCostUsd ?? budgetPolicy.maxCodegenCostUsd, budgetPolicy.maxCodegenCostUsd);
        const budgetCheck = budgetCheckForProjectedCost(store, budgetPolicy, projectedCostUsd, "Generated-node build");
        if (!budgetCheck.ok) {
            const ledger = store.saveBudgetLedger(createBudgetLedger({
        ...
        const correlationId = correlationIdForRequest(request);
        const job = requestJobId(request) && store.getJob(requestJobId(request) ?? "")
            ? store.getJob(requestJobId(request) ?? "")
            : store.saveJob(createJob({
                type: "build.codegen-node",
                workflowId: sourceWorkflow.id,
                revisionId: branchHead?.id ??
        ...
        const runningJob = store.appendJobEvent(store.saveJob({
            ...job,
            status: "running",
            startedAt: job.startedAt ?? startedAt,
            updatedAt: startedAt
        }).id, createJobEvent(job, "info", "Generated-node build loop started.", {
            nodeId: node.id
        }));
        const jobSignal = jobSupervisor.startJob(runningJob.id);
        const workspace = store.saveWorkspace(createWorkflowWorkspace({
            jobId: runningJob.id,
            workflowId: sourceWorkflow.id,
            branchId: branch?.id,
            revisionId: branchHead?.id ??
        ...
        try {
            ...
            await artifactStore.putManifest(createArtifactManifest({
                workflowId: stored.workflow.id,
            ...
            const testReport = store.saveGeneratedNodeTestReport(createGeneratedNodeTestReport({
                workflowId: stored.workflow.id,
                branchId: branch?.id,
                nodeId: node.id,
                jobId: runningJob.id,
                ...
                logs: result.logs,
                failureMessage: result.status === "failed"
                    ? result.findings.map((finding) => finding.message).join("; ") ||
                        "Generated-node eval failed."
                    : undefined
            }));
            const evalReport = store.saveGeneratedNodeEvalReport(createGeneratedNodeEvalReport({
                workflowId: stored.workflow.id,
                branchId: branch?.id,
                nodeId: node.id,
                jobId: runningJob.id,
            ...
            const updatedWorkspace = store.saveWorkspace(createUpdatedWorkspaceRecord(workspace, {
            ...
            if (result.status === "failed") {
                ...
                const failedJob = store.appendJobEvent(store.saveJob({
                    ...currentJob,
                    status: "failed",
                    workspaceId: updatedWorkspace.id,
                    updatedAt: finishedAt,
                    finishedAt,
                    error: evalReport.findings.map((finding) => finding.message).join("; ") ||
                        "Generated-node eval failed.",
                    result: {
                        workspaceId: updatedWorkspace.id,
                        testReportId: testReport.id,
                ...
                }).id, createJobEvent(currentJob, "error", "Generated-node build loop failed eval.", {
                    workspaceId: updatedWorkspace.id,
                    testReportId: testReport.id,
                    evalReportId: evalReport.id
                }));
            ...
            const workflowWithGeneratedNode = {
                ...sourceWorkflow,
                approval: null,
                updatedAt: new Date().toISOString(),
                nodes: sourceWorkflow.nodes.map((candidate) => candidate.id === node.id
                    ? {
                        ...candidate,
                        config: {
                            ...candidate.config,
            ...
            if (isTerminalJobStatus(currentJob.status)) {
                return reply.code(currentJob.status === "cancelled" ? 409 : 500).send({
                    ...
                    message: currentJob.error ?? currentJob.cancellationReason ?? "Codegen build did not complete."
                });
            }
            const completedJob = store.appendJobEvent(store.saveJob({
                ...currentJob,
                status: "succeeded",
                workspaceId: updatedWorkspace.id,
                updatedAt: finishedAt,
            ...
            }).id, createJobEvent(runningJob, "info", "Generated-node build loop completed.", {
                draftRevisionId: draftRevision.id,
                workspaceId: updatedWorkspace.id,
                testReportId: testReport.id,
                evalReportId: evalReport.id
            ...
            if (error instanceof JobCancelledError || currentJob.status === "cancelled") {
                const cancelled = jobSupervisor.cancelJob(runningJob.id, error instanceof Error ? error.message : "Codegen build cancelled.", finishedAt);
                return reply.code(409).send({
                    ok: false,
                    error: "JOB_CANCELLED",
            ...
            const failedJob = store.appendJobEvent(store.saveJob({
                ...
                updatedAt: finishedAt,
                finishedAt,
                error: error instanceof Error ? redactSecretString(error.message) : "Codegen build failed."
            }).id, createJobEvent(runningJob, "error", "Generated-node build loop failed."));
            return reply.code(500).send({
                ok: false,
    ...
    app.get("/api/workflows/:id/codegen/:nodeId/evals", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
    ...
    app.post("/api/workflows/:id/codegen/:nodeId/review", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
    ...
    app.post("/api/workflows/:id/codegen/:nodeId/promote", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
        ...
        const loadedSkill = JSON.parse(await artifactStore.readArtifact(storedArtifact.ref));
    ...
    app.post("/api/workflows/:id/runs", async (request, reply) => {
        ...
        if (runBranch?.status === "archived") {
            return reply.code(409).send(archivedBranchApiError(runBranch));
        }
        ...
        const deployedRunnerConfig = requestedDeployment ??
            latestDeployedRunnerConfiguration(store, approvedRevision.workflowId, approvedRevision.id, approvedRevision.branchId);
        if (!deployedRunnerConfig ||
            ...
            return reply.code(409).send({
                ...
                message: "Production runs require an active runner.configuration deployment for the approved revision."
            });
        }
        try {
            const correlationId = correlationIdForRequest(request);
            ...
            const job = existingJob &&
                ...
                ? store.appendJobEvent(store.saveJob({
                    ...existingJob,
                    workflowId: approvedRevision.workflowId,
                    ...
                    updatedAt: now
                }).id, createJobEvent(existingJob, "info", "Attached queued workflow run.", {
                    runId
                }))
                : store.saveJob(createJob({
                    type: "run.workflow",
    ...
    app.post("/api/workflows/:id/runs/:runId/replay", async (request, reply) => {
        ...
        const deployedRunnerConfig = latestDeployedRunnerConfiguration(store, sourceRun.workflowId, sourceRun.approvedRevisionId, sourceRun.branchId);
        if (!deployedRunnerConfig) {
            return reply.code(409).send({
                ok: false,
                error: "WORKFLOW_RUN_REQUIRES_DEPLOYMENT",
        ...
        const job = store.saveJob(createJob({
            type: "run.workflow",
            workflowId: sourceRun.workflowId,
            branchId: sourceRun.branchId,
            revisionId: sourceRun.approvedRevisionId,
    ...
    app.get("/api/workflows/:id/runs/:runId/events", async (request, reply) => {
        const run = store.getRun(request.params.runId);
        if (!run) {
            return reply.code(404).send({
                ok: false,
    ...
    app.get("/api/workflows/:id/memory", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
    ...
    app.delete("/api/workflows/:id/memory/:memoryId", async (request, reply) => {
        const memory = store
    ...
    app.get("/api/workflows/:id/audit", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
    ...
    app.get("/api/workflows/:id/revisions/:revisionId", async (request, reply) => {
        ...
        const workflowId = revision.approvedRevision?.workflowId ?? revision.draftRevision?.workflowId ?? null;
    ...
    app.post("/api/workflows/:id/revisions", async (request, reply) => {
        try {
    ...
    app.get("/api/workflows/:id/deployments", async (request) => ({
        ok: true,
        deployments: store.listDeployments(request.params.id)
    }));
    app.get("/api/workflows/:id/runtime-truth", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({
                ok: false,
    ...
    app.get("/api/workflows/:id/budget", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
    ...
    app.patch("/api/workflows/:id/budget", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
        ...
        const updated = store.saveBudgetPolicy({
            ...
            expensiveRetryConfirmationUsd: request.body.expensiveRetryConfirmationUsd ?? policy.expensiveRetryConfirmationUsd,
    ...
    app.get("/api/workflows/:id/alerts", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
    ...
    app.patch("/api/workflows/:id/alerts", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
    ...
    app.get("/api/workflows/:id/retention", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
    ...
    app.patch("/api/workflows/:id/retention", async (request, reply) => {
        if (!store.getWorkflow(request.params.id)) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_NOT_FOUND",
        ...
        const updated = store.saveRetentionPolicy({
            ...
            maxSuccessfulRunWorkspaceDays: request.body.maxSuccessfulRunWorkspaceDays ?? current.maxSuccessfulRunWorkspaceDays,
            maxFailedRunWorkspaceDays: request.body.maxFailedRunWorkspaceDays ?? current.maxFailedRunWorkspaceDays,
    ...
    app.get("/api/workflows/:id/agent-timeline", async (request) => ({
        ok: true,
        events: store.listAgentTimelineEvents(request.params.id)
    }));
    app.get("/api/workflows/:id/decision-traces", async (request) => ({
        ok: true,
        traces: store.listNodeDecisionTraces(request.params.id)
    }));
    app.get("/api/workflows/:id/nodes/:nodeId/decision-traces", async (request) => ({
        ok: true,
        traces: store.listNodeDecisionTraces(request.params.id, request.params.nodeId)
    }));
    app.get("/api/workflows/:id/decision-traces/export", async (request) => {
    ...
    app.get("/api/workflows/:id/audit/export", async (request) => {
    ...
    app.get("/api/workflows/:id/deployments/active", async (request) => ({
        ok: true,
        ...deploymentActivationSummary(store, request.params.id)
    }));
    app.get("/api/workflows/:id/schedules", async (request) => ({
        ok: true,
        schedules: store.listSchedules(request.params.id)
    }));
    app.post("/api/workflows/:id/schedules/:scheduleId/pause", async (request, reply) => {
    ...
    app.post("/api/workflows/:id/schedules/:scheduleId/resume", async (request, reply) => {
        const schedule = store.getSchedule(request.params.scheduleId);
        if (!schedule || schedule.workflowId !== request.params.id) {
            return reply.code(404).send({
                ok: false,
    ...
    app.post("/api/workflows/:id/deployments", async (request, reply) => {
        const approvedRevision = store.getApprovedRevision(request.body.approvedRevisionId);
        ...
        const latestEvaluation = store.getLatestDraftEvaluation(request.params.id, deploymentBranchId);
        ...
        const missingSecrets = collectSecretRefs(approvedRevision.workflow)
            ...
            .filter((secretName) => !secretStore.listSecrets().some((secret) => secret.name === secretName));
        ...
        const blockedIntegrations = requiredIntegrations.filter((integration) => readiness.find((item) => item.id === integration)?.ready !== true);
        ...
        updateRequestJob(store, request, "running", "Workflow deployment started.", {
            approvedRevisionId: approvedRevision.id,
        ...
        updateRequestJob(store, request, "succeeded", "Workflow deployment completed.", {
    ...
    app.post("/api/workflows/:id/deployments/:deploymentId/undeploy", async (request, reply) => {
    ...
    app.post("/api/workflows/:id/deployments/:deploymentId/rollback", async (request, reply) => {
        const deployments = store.listDeployments(request.params.id);
        const deployment = deployments.find((candidate) => candidate.id === request.params.deploymentId);
        if (!deployment) {
            return reply.code(404).send({
                ok: false,
                error: "WORKFLOW_DEPLOYMENT_NOT_FOUND",
        ...
        const rollbackTarget = deployments
            .filter((candidate) => candidate.id !== deployment.id &&
        ...
        const updated = store.saveDeployment({
            ...
            metadata: {
                ...
                rollbackTarget: jsonRecord(rollbackTarget
    ...
    app.post("/api/workflows/:id/executions", async (request, reply) => {
        const stored = store.getWorkflow(request.params.id);
        if (!stored) {
            return reply.code(404).send({ error: "WORKFLOW_NOT_FOUND" });
        }
...
function createClarificationRequestIfNeeded(request, correlationId) {
...
function promptNeedsClarification(prompt) {
...
function hasRequiredClarificationAnswers(request) {
...
function enrichPlanRequestWithClarifications(request) {
    const answers = usableClarificationAnswers(request.clarificationAnswers);
    if (answers.length === 0) {
        return request;
    }
...
function usableClarificationAnswers(answers) {
...
function isValidateRequest(input) {
...
function isPublicRoute(method, url) {
    ...
    return (pathname === "/health" || (method === "GET" && pathname === "/api/integrations/google/callback"));
}
...
function publicSecretMetadata(secretStore) {
    return secretStore
        .listSecrets()
...
async function exchangeGoogleOAuthCode(code) {
    ...
    const response = await fetch(process.env.GOOGLE_TOKEN_URL ?? "https://oauth2.googleapis.com/token", {
        method: "POST",
        headers: {
...
async function revokeGoogleOAuthSecret(secret) {
...
function googleTokenForRevocation(secret) {
    try {
...
async function validateApprovalReadiness(store, workflow, artifactStore, branchId) {
    const issues = [];
    ...
    for (const [index, node] of workflow.nodes.entries()) {
        ...
        const latestEval = store
            ...
            .filter((report) => report.status === "passed" && (branchId === undefined || report.branchId === branchId))
            .at(-1);
        if (!latestEval) {
            issues.push({
        ...
        else if (!latestEval.schemaValid ||
...
function validatePlanningReadiness(workflow) {
...
function rehydratePromotedSkills(artifactStore) {
    ...
    const skills = index.artifacts.map((ref) => JSON.parse(readLocalCodegenArtifact(artifactStore.root, ref)));
    loadPromotedSkills(skills);
}
function recordPromotedSkillArtifact(artifactStore, ref) {
    if (!(artifactStore instanceof LocalCodegenArtifactStore)) {
        return;
    }
    const indexPath = promotedSkillIndexPath(artifactStore);
    const existing = existsSync(indexPath)
        ...
        : [];
    const artifacts = [...existing, ref].filter((artifact, index, values) => values.findIndex((candidate) => candidate.path === artifact.path && candidate.checksum === artifact.checksum) === index);
...
function promotedSkillIndexPath(artifactStore) {
    return join(artifactStore.root, "promoted-skills", "index.json");
}
function parsePromotedSkillIndex(content) {
    const parsed = JSON.parse(content);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        throw new Error("Promoted skill index must be a JSON object.");
    }
    const record = parsed;
    if (record.kelpclawPromotedSkillIndexVersion !== "1.0.0" ||
...
function readLocalCodegenArtifact(root, ref) {
...
function isWorkflowCodegenArtifactRef(value) {
...
function agentRunAuditAnchorPath(runId) {
    return join(process.env.KELPCLAW_AUDIT_ANCHOR_DIR ?? join(process.cwd(), ".kelpclaw", "audit-anchors"), `${slugify(runId)}.jsonl`);
}
async function publishAuditAnchorFromEnv(anchor) {
...
function parseApiRoleList(value) {
    const rawRoles = typeof value === "string"
        ? value
            .split(",")
            .map((role) => role.trim())
...
function parseRoleTokenExpiry(expiresAt, ttlSeconds) {
...
function createPromotedSkill(workflow, node) {
...
function createPromotedSkillFromTrajectory(run, workflow, input) {
    ...
    return {
        ...
        requiredSecrets: [],
        adapterDependencies: [
            ...new Set(run.events.map((event) => event.toolName).filter((tool) => tool.startsWith("adapter.")))
        ].sort(),
        adapterOperations: [],
        ...
        examples: [
            {
                ...
                output: lastStep?.result && typeof lastStep.result === "object" && !Array.isArray(lastStep.result)
                    ? lastStep.result
...
function firstUnresolvedPolicyApproval(run) {
    return run.events.find((event) => event.status === "pending" &&
...
function promotionCapability(node) {
    const text = `${node.label} ${node.description} ${node.codegen?.latestPrompt ?? ""}`.toLowerCase();
...
function trajectoryCapability(run) {
...
function createGeneratedModuleSignature(node) {
    if (!node.codegen) {
    ...
    return {
        ...
        artifactHash: hashReusableModuleValue(node.codegen.artifacts
            .map((artifact) => ({
            path: artifact.path,
            checksum: artifact.checksum,
            contentType: artifact.contentType
...
function generatedModuleSignaturesMatch(left, right) {
    return stableJsonStringify(left) === stableJsonStringify(right);
}
function hashReusableModuleValue(value) {
    return checksumArtifactContent(typeof value === "string" ? value : stableJsonStringify(value));
}
function finalizeBranchDraftWithReuse(store, input) {
    ...
    const reuse = applyGeneratedModuleReuse(store, input.workflowId, input.branch.id, draftRevision.workflow);
    const reuseDecisions = reuse.decisions.map((decision) => store.saveGeneratedModuleReuseDecision(decision));
    if (reuse.changed) {
...
function applyGeneratedModuleReuse(store, workflowId, branchId, workflow) {
    const decisions = computeGeneratedModuleReuseDecisions(store, workflowId, branchId, workflow);
    const reusable = new Map(decisions
    ...
    const nodes = workflow.nodes.map((node) => {
        ...
        const sourceNode = sourceDraft?.workflow.nodes.find((candidate) => candidate.id === node.id && candidate.kind === "codegen");
...
function computeGeneratedModuleReuseDecisions(store, workflowId, branchId, workflow) {
    return workflow.nodes
        .filter((node) => node.kind === "codegen" && node.codegen)
        .map((node) => {
        const signature = createGeneratedModuleSignature(node);
        const candidate = findGeneratedModuleReuseCandidate(store, workflowId, branchId, node, signature);
        const now = new Date().toISOString();
        if (candidate?.matching) {
            return {
                ...
                signature,
                gates: [],
                reason: "Generated module signature matches a passed module from another branch; branch-local re-evaluation is required.",
        ...
        return {
            ...
            branchId,
            nodeId: node.id,
            status: candidate ? "blocked-drift" : "regenerate",
            createdAt: now,
            sourceBranchId: candidate?.branch.id,
            sourceDraftRevisionId: candidate?.draft.id,
            signature,
...
function findGeneratedModuleReuseCandidate(store, workflowId, branchId, node, signature) {
    let fallback;
    for (const branch of store.listBranches(workflowId)) {
        ...
        const candidate = draft?.workflow.nodes.find((item) => item.id === node.id && item.kind === "codegen" && item.codegen);
...
function reuseMismatchGates(left, right) {
    const gates = [];
    if (left.promptHash !== right.promptHash) {
        gates.push("prompt");
    }
    if (left.inputSchemaHash !== right.inputSchemaHash ||
...
function createBranchMergePreview(store, workflowId, sourceBranchId, input) {
    ...
    const selectedChangeIds = input.mode === "cherry-pick" ? new Set(input.cherryPickChangeIds ?? []) : undefined;
    ...
    const validationConflicts = validation.ok
        ? []
        : validation.errors.map((issue, index) => ({
            id: `conflict.validation.${index}`,
            kind: "validation-blocked",
            elementKind: issue.path[0] === "edges" ? "edge" : issue.path[0] === "nodes" ? "node" : "workflow",
            path: issue.path,
...
function mergeWorkflowGraphs(input) {
    const resolutionMap = new Map(input.resolutions.map((resolution) => [resolution.conflictId, resolution]));
...
function sourceValueForMerge(input, id, baseValue) {
    if (!input.selectedChangeIds) {
        return input.source.get(id);
    }
    const selected = input.sourceChanges.some((change) => change.elementId === id &&
        change.kind.startsWith(input.kind) &&
        input.selectedChangeIds?.has(change.id));
    return selected ? input.source.get(id) : baseValue;
}
function automaticMergeValue(baseValue, sourceValue, targetValue) {
...
function conflictForElement(elementKind, id, baseValue, sourceValue, targetValue) {
    if (sameJson(sourceValue, baseValue) ||
    ...
    const bothAdded = baseValue === undefined && sourceValue !== undefined && targetValue !== undefined;
...
function classifyMergeConflict(baseValue, sourceValue, targetValue) {
    if (isWorkflowNode(baseValue) || isWorkflowNode(sourceValue) || isWorkflowNode(targetValue)) {
        ...
        if (!sameJson(sourceNode?.inputs, targetNode?.inputs) ||
...
function resolveConflictValue(resolution, conflict) {
...
function graphDiffToSpecDiff(before, after) {
    return createWorkflowSpecDiff(before, after);
}
function defaultFixturePayload(schemas) {
...
function createNanoClawRunner(store) {
...
function createRegisteredAdapters(store, mock) {
    const adapters = mock
...
function createAgentMemoryAccess(store) {
    return {
        list(input) {
            ...
            switch (input.memoryScope) {
                case "none":
                    return [];
                case "node":
                    return records.filter((record) => record.scope === "node" &&
                        record.workflowId === input.workflowId &&
                        record.nodeId === input.nodeId);
                case "workflow":
                    return records.filter((record) => (record.scope === "workflow" || record.scope === "node") &&
                        record.workflowId === input.workflowId &&
...
async function executeWorkflowRunJob(input) {
    ...
    const deployment = deploymentId === undefined
        ? latestDeployedRunnerConfiguration(input.store, approvedRevision.workflowId, approvedRevision.id, approvedRevision.branchId)
        : input.store
    ...
    try {
        const result = await executeCompiledDag(dag, input.runner ?? createNanoClawRunner(input.store), {
            ...
            signal: input.signal,
            checkpointStore: {
                getCheckpoint: ({ runId: lookupRunId, nodeId, inputHash }) => input.store.getRunCheckpoint(lookupRunId, nodeId, inputHash),
        ...
        recordRuntimeNodeDecisionTraces(input.store, approvedRevision.workflow, approvedRevision.id, completed, correlationId);
        recordRunAuditRecords(input.store, approvedRevision.workflow, approvedRevision.id, completed, correlationId);
        if (result.status === "failed") {
            await recordRunFailureAlert(input.store, input.secretStore, completed, approvedRevision.id, correlationId);
        }
        return {
            result: {
                runId,
        ...
        await recordRunFailureAlert(input.store, input.secretStore, failed, approvedRevision.id, correlationId);
        throw error;
    }
}
function correlationIdForRequest(request) {
...
function createWorkflowWorkspace(input) {
    ...
    const mounts = mountedAgents.map((role) => {
        ...
        return {
            role,
            path: mountPath,
            mode: role === "workflow-architect" ? "ro" : "rw"
        };
    });
    const workspace = {
        id,
...
function createUpdatedWorkspaceRecord(workspace, input) {
    const artifactRefs = input.artifacts.map((artifact) => {
        resolveWorkspacePath(workspace.rootPath, artifact.path);
        return artifact;
    });
    const filePaths = input.files.map((file) => normalizeWorkspaceRelativePath(workspace.rootPath, file));
...
function deploymentKindBlocker(kind, workflow) {
    const approvedCodegenNodes = workflow.nodes.filter((node) => node.kind === "codegen" && node.codegen?.review.status === "approved");
    if ((kind === "skill.publication" || kind === "generated.service") &&
        approvedCodegenNodes.length === 0) {
        return `${kind} requires at least one approved generated node.`;
    }
    if (kind === "schedule.activation" &&
    ...
    switch (input.deployment.kind) {
        ...
        case "integration.configuration": {
            const bindings = requiredIntegrationsForWorkflow(workflow).map((integration) => ({
                ...
                integration,
                status: "ready",
                secretRefs: collectSecretRefs(workflow).filter((secretRef) => secretRef.toLowerCase().includes(integration))
            }));
...
async function writeDeploymentArtifact(input, filename, payload) {
    const stored = await input.artifactStore.putArtifact(createGeneratedArtifact({
...
function deploymentActivationSummary(store, workflowId) {
    const activeDeployments = store
        .listDeployments(workflowId)
        .filter((deployment) => deployment.status === "deployed");
    return {
        activeDeployments,
        activeSchedules: activeDeployments.flatMap((deployment) => jsonArrayMetadata(deployment.metadata.activeScheduleRegistrations)),
        runnerConfigurations: activeDeployments.flatMap((deployment) => jsonObjectMetadata(deployment.metadata.runnerConfig)
            ? [deployment.metadata.runnerConfig]
            : []),
        skillPublications: activeDeployments.flatMap((deployment) => jsonArrayMetadata(deployment.metadata.skillPublications)),
        integrationBindings: activeDeployments.flatMap((deployment) => jsonArrayMetadata(deployment.metadata.integrationBindings)),
        bundles: activeDeployments.flatMap((deployment) => jsonObjectMetadata(deployment.metadata.bundle)
            ? [deployment.metadata.bundle]
            : []),
        generatedServices: activeDeployments.flatMap((deployment) => jsonObjectMetadata(deployment.metadata.generatedServiceConfig)
            ? [deployment.metadata.generatedServiceConfig]
            : [])
    };
}
function registerSchedulesForDeployment(store, deployment, approvedRevision) {
    const now = new Date();
...
function disableSchedulesForDeployment(store, deploymentId, status) {
    const now = new Date().toISOString();
...
function nextCronFire(cron, timezone, currentDate = new Date()) {
...
function revisionFromApprovedRevisionId(approvedRevisionId) {
...
function createDeploymentRollbackTarget(deployment, previousDeployment) {
    return {
        deploymentId: deployment.id,
        workflowId: deployment.workflowId,
        ...(deployment.branchId ? { branchId: deployment.branchId } : {}),
...
function deploymentArtifactRefs(deployment) {
    return [
...
function isWorkflowArtifactRef(value) {
    return (typeof value.path === "string" &&
        typeof value.checksum === "string" &&
        typeof value.contentType === "string");
}
function latestDeployedRunnerConfiguration(store, workflowId, approvedRevisionId, branchId) {
    return store
        .listDeployments(workflowId)
...
function jsonArrayMetadata(value) {
    return Array.isArray(value) ? value.filter(jsonObjectMetadata) : [];
}
function jsonObjectMetadata(value) {
    return !!value && typeof value === "object" && !Array.isArray(value);
}
function jsonRecord(value) {
    return JSON.parse(JSON.stringify(value));
}
function providerRuntimeConfigsFromEnv() {
    ...
    for (const role of [
        ...
        configs.push(providerRuntimeConfig({
            role,
            provider: codegenProvider,
...
function providerRuntimeConfig(input) {
    const missingCredential = input.provider === "deterministic"
        ? undefined
...
function hasOpenAiResponsesCredential() {
    return Boolean(process.env.OPENAI_API_KEY || resolveAzureOpenAiResponsesConfig());
}
function providerFromEnv(key, fallback = "anthropic") {
...
function modelForProviderRole(role, provider) {
    if (provider === "deterministic") {
        return "deterministic";
    }
    if (provider === "openai") {
        return (openAiRoleModel(role) ??
    ...
    if (provider === "openweight") {
        return (openWeightRoleModel(role) ??
    ...
    return (anthropicRoleModel(role) ??
        process.env.KELPCLAW_CODEGEN_MODEL ??
        process.env.KELPCLAW_PLANNER_MODEL ??
        "claude-sonnet-4-5-20250929");
}
function openAiRoleModel(role) {
    switch (role) {
        case "planner":
...
function openWeightRoleModel(role) {
    switch (role) {
        case "planner":
...
function anthropicRoleModel(role) {
    switch (role) {
        case "planner":
...
function defaultBudgetPolicy(workflowId, branchId) {
    return {
        workflowId,
        ...(branchId ? { branchId } : {}),
...
function ensureBudgetPolicy(store, workflowId, branchId) {
    return (store.getBudgetPolicy(workflowId, branchId) ??
        store.saveBudgetPolicy(defaultBudgetPolicy(workflowId, branchId)));
}
function defaultAlertPolicy(workflowId, branchId) {
    return {
        workflowId,
        ...(branchId ? { branchId } : {}),
        enabled: false,
...
function ensureAlertPolicy(store, workflowId, branchId) {
    return (store.getAlertPolicy(workflowId, branchId) ??
        store.saveAlertPolicy(defaultAlertPolicy(workflowId, branchId)));
}
function defaultRetentionPolicy(workflowId, branchId) {
    return {
        workflowId,
        ...(branchId ? { branchId } : {}),
        maxRunEventDays: 14,
...
function ensureRetentionPolicy(store, workflowId, branchId) {
    return (store.getRetentionPolicy(workflowId, branchId) ??
        store.saveRetentionPolicy(defaultRetentionPolicy(workflowId, branchId)));
}
async function recordRunFailureAlert(store, secretStore, run, approvedRevisionId, correlationId) {
    ...
    for (const channel of policy.channels) {
        const delivered = await deliverAlertChannel(policy, secretStore, run, channel, approvedRevisionId);
        deliveryEvents.push(createStructuredRunEvent({
...
async function deliverAlertChannel(policy, secretStore, run, channel, approvedRevisionId) {
    if (channel === "webhook") {
    ...
    const secretRef = policy.secretRefs[secretName] ??
...
function sanitizePerAgentBudget(value) {
    return Object.fromEntries(Object.entries(value).filter((entry) => {
        const amount = entry[1];
        return typeof amount === "number" && Number.isFinite(amount) && amount >= 0;
    }));
}
function budgetCheckForProjectedCost(store, policy, projectedCostUsd, operation) {
    ...
    if (projectedCostUsd > remainingCostUsd) {
        return {
            ok: false,
            remainingCostUsd,
            reason: `${operation} is blocked because projected cost $${projectedCostUsd.toFixed(4)} exceeds remaining workflow budget $${remainingCostUsd.toFixed(4)}.`
        };
    }
    return { ok: true, remainingCostUsd };
}
function workflowActualCostUsd(store, workflowId) {
    return store
        .listAgentRuns(workflowId)
        .reduce((total, run) => total + (run.costUsd ?? agentRunInvocationCostUsd(run)), 0);
}
function agentRunInvocationCostUsd(run) {
    return (run.modelInvocations ?? []).reduce((total, invocation) => total + (invocation.costUsd ?? 0), 0);
}
...
function saveBudgetLedgersForAgentRuns(store, policy, agentRuns, jobId) {
    let cumulativeActualCostUsd = workflowActualCostUsd(store, policy.workflowId);
    const jobActualCostUsd = agentRuns.reduce((total, run) => total + (run.costUsd ?? agentRunInvocationCostUsd(run)), 0);
    const remainingAfterJob = Math.max(0, policy.maxWorkflowCostUsd - cumulativeActualCostUsd);
    store.saveBudgetLedger(createBudgetLedger({
    ...
    for (const run of agentRuns) {
        ...
        store.saveBudgetLedger(createBudgetLedger({
            ...
            retryEstimateUsd: policy.expensiveRetryConfirmationUsd,
            status: actualCostUsd >= policy.expensiveRetryConfirmationUsd
                ? "confirmation-required"
                : "within-budget"
        }));
    }
}
function saveTimelineEventsForAgentRuns(store, agentRuns, branchId) {
...
function timelineDecisionForRun(run) {
    if (run.error) {
        return run.error;
    }
    if (run.role === "fixer") {
...
function fixTriageActionFromSummary(summary) {
    if (summary.includes("smallest local patch"))
        return "targeted-patch";
    if (summary.includes("Regenerate"))
        return "retry-codegen";
    if (summary.includes("Rebuild from a revised architecture"))
        return "rearchitect";
    if (summary.includes("External blocker"))
        return "give-up";
    return undefined;
}
function codegenDecisionKindForRole(role) {
    switch (role) {
        case "workflow-architect":
...
function codegenAlternativesForRun(run) {
    if (run.role === "fixer") {
        return [
...
function codegenSelectedAction(run) {
...
function plannerAlternativesForNode(node, route) {
    const alternatives = [
...
function runtimeTruthSnapshot(store, workflowId, branchId) {
    ...
    const acceptedDraft = stored.draftRevisions
        .filter((revision) => (!branch?.id || revision.branchId === branch.id) && revision.source === "plan-accepted")
        .at(-1);
...
function createAuditExportRecord(store, workflowId) {
    const auditRecords = store.listAuditRecords(workflowId).map((record) => redactJsonRecord(jsonRecord({
        recordType: "audit",
...
function createDecisionTraceExportRecord(store, workflowId) {
    const traces = store.listNodeDecisionTraces(workflowId);
    const evalExamples = traces.map(decisionTraceEvalExample);
    const records = [
        ...traces.map((trace) => redactJsonRecord(jsonRecord({ recordType: "node-decision-trace", ...trace }))),
        ...evalExamples.map((example) => redactJsonRecord(jsonRecord({ recordType: "decision-trace-eval-example", ...example })))
    ];
    return {
...
function decisionTraceEvalExample(trace) {
    const event = trace.events.at(-1);
    const outcome = event.evalOutcome === "passed"
        ? "pass"
...
function archivedBranchApiError(branch) {
    return {
        ok: false,
...
function requiredIntegrationsForWorkflow(workflow) {
...
function createJobEvent(job, level, message, metadata) {
    return {
...
class ApiJobSupervisor {
    ...
    cancelJob(jobId, reason, cancelledAt = new Date().toISOString()) {
        const job = this.store.getJob(jobId);
        if (!job) {
        ...
        const withEvent = cancelled.events.at(-1)?.message === "Job cancelled."
            ? cancelled
            : this.store.appendJobEvent(cancelled.id, createJobEvent(cancelled, "error", "Job cancelled.", { reason }));
...
class ApiJobWorker {
    ...
    async runClaimedJob(job) {
        ...
        const claimed = this.store.appendJobEvent(job.id, createJobEvent(job, "info", "Job claimed by local worker.", {
        ...
        try {
            ...
            const finishedAt = new Date().toISOString();
            this.store.appendJobEvent(this.store.saveJob({
                ...current,
                status: "succeeded",
                updatedAt: finishedAt,
                finishedAt,
                ...(output?.result ? { result: output.result } : {})
            }).id, createJobEvent(current, "info", "Worker job completed.", output?.result));
        }
        catch (error) {
            const current = this.store.getJob(job.id) ?? claimed;
            if (error instanceof JobCancelledError || current.status === "cancelled") {
                this.supervisor.cancelJob(job.id, error instanceof Error ? error.message : "Worker job cancelled.");
                return;
            }
            const finishedAt = new Date().toISOString();
            ...
            const nextRunAt = retryable
                ? new Date(Date.now() +
                    (current.retry.backoffSeconds ?? Math.min(60, current.retry.attempt * 2)) * 1000).toISOString()
                : undefined;
            ...
            this.store.appendJobEvent(failed.id, createJobEvent(failed, "error", retryable ? "Worker job scheduled for retry." : "Worker job failed.", {
...
class ApiRetentionCleanupWorker {
    ...
    cleanupRunEvents(workflowId, policy, now) {
    ...
    cleanupWorkspaces(workflowId, policy, now) {
        for (const workspace of this.store.listWorkspaces(workflowId)) {
            if (workspace.retentionStatus !== "active") {
                continue;
            }
            const retentionDays = workspace.retentionPolicy === "retain-on-failure"
...
function opsHealth(store, jobWorker, scheduleWorker, latestRouterEvalRun) {
    ...
    const expiredMemories = allMemories.filter((memory) => memory.expiresAt !== undefined && Date.parse(memory.expiresAt) <= Date.now()).length;
    return {
        ...
        scheduler: {
            ...
            dueSchedules: schedules.filter((schedule) => schedule.status === "active" && Date.parse(schedule.nextFireAt) <= dueNow).length
        },
        runs: {
            running: runningRuns,
...
function recoverInterruptedJobs(store) {
    const now = new Date().toISOString();
    for (const job of store.listJobs()) {
        ...
        store.appendJobEvent(failed.id, createJobEvent(failed, "error", "Running job was interrupted by API restart.", {
            retryable,
            ...(nextRunAt ? { nextRunAt } : {})
        }));
    }
...
function collectWorkflowIds(store) {
    return [
        ...new Set([
...
function sleepWithSignal(durationMs, signal) {
    if (durationMs <= 0) {
...
function throwIfRequestJobCancelled(store, supervisor, request) {
...
function updateRequestJob(store, request, status, message, metadata, supervisor) {
    ...
    return store.appendJobEvent(updated.id, createJobEvent(updated, status === "failed" || status === "cancelled" ? "error" : "info", message, metadata));
}
function requestJobId(request) {
    const jobIdHeader = request.headers["x-kelpclaw-job-id"];
...
function recordPlannerNodeDecisionTraces(store, input) {
    const now = new Date().toISOString();
    ...
    const changedNodeIds = new Set(input.changedNodeIds ?? input.workflow.nodes.map((node) => node.id));
    const traces = [];
    for (const node of input.workflow.nodes.filter((candidate) => changedNodeIds.has(candidate.id))) {
        const kind = input.source === "reprompt" ? "planner.node-updated" : "planner.node-created";
        ...
        const affectedEdges = input.workflow.edges
            ...
            .map((edge) => edge.id);
        const rationale = node.codegen?.plannerRationale ??
            node.description ??
...
function recordCodegenNodeDecisionTraces(store, input) {
    const traces = [];
    let cumulativeCostUsd = 0;
...
function recordRuntimeNodeDecisionTraces(store, workflow, approvedRevisionId, run, correlationId) {
    const traces = [];
    for (const nodeResult of run.result?.nodeResults ?? []) {
        ...
        const role = node?.agentic ? "agentic-node-designer" : "runner";
        const events = runtimeEvents.map((runtimeEvent) => ({
            id: `${traceId}.event.${randomUUID()}`,
            traceId,
            workflowId: workflow.id,
            ...
            affectedEdgeIds: workflow.edges
                .filter((edge) => edge.source.nodeId === nodeResult.nodeId || edge.target.nodeId === nodeResult.nodeId)
                .map((edge) => edge.id),
            constraints: jsonRecord({
                runId: run.id,
            ...
            outputArtifactRefs: [],
            evalOutcome: nodeResult.status === "failed" ? "failed" : "passed",
            metadata: jsonRecord({
                correlationId
            })
        }));
...
function runtimeTraceKindFromJson(value) {
...
function persistCodegenArtifactManifests(store, workflow, revisionId, createdAt) {
    return workflow.nodes
        .filter((node) => node.kind === "codegen" && node.codegen)
        .map((node) => {
        ...
        return store.saveArtifactManifest({
            ...
            createdAt,
            artifacts,
            manifestChecksum: checksumArtifactContent(stableJsonStringify(artifacts))
        });
    });
}
function persistWorkflowHandoffArtifact(store, approvedRevision, correlationId) {
    const workflow = approvedRevision.workflow;
    const handoff = redactJsonRecord(jsonRecord({
    ...
    store.saveArtifactManifest({
        ...
        artifacts: [artifact],
        manifestChecksum: checksumArtifactContent(stableJsonStringify([artifact]))
    });
    return artifact;
}
function collectSecretRefs(workflow) {
    return [
        ...new Set(workflow.nodes.flatMap((node) => Object.values(node.secretRefs ?? {}).filter((secretRef) => secretRef.length > 0)))
    ].sort();
}
function collectCodegenArtifactRefs(workflow) {
    return workflow.nodes.flatMap((node) => node.codegen?.artifacts ?? []);
}
function recordRunAuditRecords(store, workflow, revisionId, run, correlationId) {
    if (!run.result) {
        return;
    }
...
function containerAuditRecord(node, result) {
    const metadata = result.metadata;
    const network = metadata?.network === "bridge"
        ? "bridge"
...
function adapterAuditRecords(node, succeeded) {
...
function deliveryAuditRecord(node, output, succeeded) {
    if (node.kind !== "delivery") {
        return undefined;
    }
    const delivery = typeof output === "object" && output !== null ? output : {};
    const nested = "delivery" in delivery && typeof delivery.delivery === "object" && delivery.delivery !== null
        ? delivery.delivery
        : delivery;
    const channels = typeof nested === "object" &&
        nested !== null &&
        "channels" in nested &&
        Array.isArray(nested.channels)
        ? nested.channels.filter((channel) => typeof channel === "string")
...
function declaredDeliveryChannels(node) {
...
function enrichRunEvents(events, context) {
...
function createStructuredRunEvent(input) {
    return input;
}
function severityForRunEvent(event) {
...
function kindForRunEvent(event) {
    if (event.nodeId) {
