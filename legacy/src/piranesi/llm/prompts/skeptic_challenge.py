from __future__ import annotations

from piranesi.llm.prompts._base import (
    Message,
    build_tool_spec,
    code_block,
    render_messages,
    schema_instruction,
)

VERSION = "1.0.0"
TOOL_NAME = "submit_skeptic_challenge"
CANARY_FRAGMENTS = (
    "You are Project Piranesi's skeptic challenge stage.",
    "Your job is to disprove the reported vulnerability "
    "whenever the code supports that conclusion.",
)
RESPONSE_SCHEMA: dict[str, object] = {
    "type": "object",
    "additionalProperties": False,
    "properties": {
        "verdict": {
            "type": "string",
            "enum": ["genuine", "false_positive", "uncertain"],
        },
        "confidence": {
            "type": "number",
            "minimum": 0.0,
            "maximum": 1.0,
        },
        "reasoning": {"type": "string"},
        "mitigations_found": {
            "type": "array",
            "items": {"type": "string"},
        },
        "remaining_risk": {"type": "string"},
    },
    "required": [
        "verdict",
        "confidence",
        "reasoning",
        "mitigations_found",
        "remaining_risk",
    ],
}
TOOL_SPEC = build_tool_spec(
    name=TOOL_NAME,
    description="Adversarially challenge a candidate vulnerability report.",
    parameters=RESPONSE_SCHEMA,
)
SYSTEM_PROMPT = "\n".join(
    [
        CANARY_FRAGMENTS[0],
        CANARY_FRAGMENTS[1],
        "Look for sanitization, framework protections, "
        "unreachable paths, and exploitability limits.",
        schema_instruction(TOOL_NAME, RESPONSE_SCHEMA),
    ]
)


def render(
    *,
    cwe_id: str,
    cwe_name: str,
    file_path: str,
    line_number: int,
    source: str,
    sink: str,
    code_context: str,
    language: str = "typescript",
) -> list[Message]:
    user_prompt = "\n".join(
        [
            "A static analyzer reported the following vulnerability. Try to disprove it.",
            "",
            "Finding:",
            f"- Vulnerability class: {cwe_id} ({cwe_name})",
            f"- File: {file_path}:{line_number}",
            f"- Taint path: {source} -> ... -> {sink}",
            "",
            "Source code context:",
            code_block(language, code_context),
        ]
    )
    return render_messages(SYSTEM_PROMPT, user_prompt)


__all__ = [
    "CANARY_FRAGMENTS",
    "RESPONSE_SCHEMA",
    "SYSTEM_PROMPT",
    "TOOL_NAME",
    "TOOL_SPEC",
    "VERSION",
    "render",
]
