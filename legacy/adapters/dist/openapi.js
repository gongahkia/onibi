...
export async function importOpenApiConnector(input) {
...
export function createOpenApiAdapter(connector, options = {}) {
    const routes = connector.operations.map((operation) => openApiRouteForOperation(operation, connector.auth[0]));
...
export async function testOpenApiConnector(connector) {
    ...
    return {
        ...
        lastTest: {
            ...
            message: connector.operations.length > 0
                ? `Imported ${connector.operations.length} OpenAPI operation(s).`
                : "OpenAPI connector has no callable operations."
        }
    };
}
async function dereferenceOpenApi(input) {
    const parser = new SwaggerParser();
    const source = input.document === undefined
        ? input.sourceUrl
    ...
    return (await parser.dereference(source));
}
...
function firstServerUrl(api, sourceUrl) {
...
function collectOpenApiOperations(api, defaultServerUrl) {
    ...
    for (const [path, pathItem] of Object.entries(paths)) {
        ...
        for (const [method, operation] of Object.entries(pathItem)) {
            ...
            operations.push({
                name: operationId,
                version: "1.0.0",
                description: stringValue(operation.description, stringValue(operation.summary, `${method.toUpperCase()} ${path}`)),
...
function openApiInputSchema(operation) {
...
function operationServerUrl(operation, pathItem, fallback) {
...
function collectAuthRequirements(api, secretRefs) {
...
function collectAllowedHosts(serverUrl, operations) {
    const hosts = new Set([new URL(serverUrl).hostname]);
...
function absolutizeServerUrl(value, sourceUrl) {
