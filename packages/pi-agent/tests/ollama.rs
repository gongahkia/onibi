...
fn ollama_load_check_runs_empty_prompt_after_guard_allows() {
    ...
    fs::write(&meminfo_path, "MemTotal:        4194304 kB\n").expect("write meminfo");
    fs::write(
        &fake_ollama,
...
#[cfg(unix)]
#[test]
fn ollama_load_check_refuses_4gb_pi_for_8gb_model_before_invoking_ollama() {
...
#[cfg(unix)]
#[test]
fn ollama_models_reports_4gb_availability() {
    let root = temp_root("ollama-models");
    fs::create_dir_all(&root).expect("create root");
    let model_path = root.join("model");
    let meminfo_path = root.join("meminfo");
    fs::write(&model_path, "Raspberry Pi 5 Model B Rev 1.0\0").expect("write model");
    ...
    let stdout: serde_json::Value =
        serde_json::from_slice(&output.stdout).expect("parse models json");
    assert_eq!(stdout["default_model"], "qwen2.5:0.5b");
    let models = stdout["models"].as_array().expect("models array");
    let default_model = models
        .iter()
        .find(|model| model["model"] == "qwen2.5:0.5b")
        .expect("default model");
    let three_b = models
        .iter()
        .find(|model| model["model"] == "llama3.2:3b")
        .expect("3b model");
    assert_eq!(default_model["available"], true);
    assert_eq!(three_b["available"], false);
