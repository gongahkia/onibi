...
fn hardening_command(args: Vec<String>) -> Result<(), ExitCode> {
    let Some(subcommand) = args.first() else {
        eprintln!(
            "usage: kelp-pi-agent hardening <render-network|apply-network|apply-scanner-targets> ..."
        );
        return Err(ExitCode::from(64));
    };
    match subcommand.as_str() {
        "apply-network" => hardening_apply_network_command(args[1..].to_vec()),
        "apply-scanner-targets" => hardening_apply_scanner_targets_command(args[1..].to_vec()),
...
fn hardening_apply_scanner_targets_command(args: Vec<String>) -> Result<(), ExitCode> {
    let mut nft_bin = PathBuf::from("nft");
    let mut targets = Vec::<Ipv4Addr>::new();
    let mut index = 0;
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--target-ip" => {
                let Some(value) = args.get(index + 1) else {
                    eprintln!("--target-ip requires a value");
                    return Err(ExitCode::from(64));
                };
                let Ok(parsed) = value.parse::<Ipv4Addr>() else {
                    eprintln!("--target-ip must be an IPv4 address");
                    return Err(ExitCode::from(64));
                };
                targets.push(parsed);
                index += 2;
    ...
    targets.sort();
    targets.dedup();
    apply_scanner_target_rules(&nft_bin, &targets).map_err(|error| {
        eprintln!("hardening apply-scanner-targets failed: {error}");
        ExitCode::from(78)
    })?;
    println!("applied {} scanner target IPs", targets.len());
    Ok(())
}
...
fn ollama_check_command(args: Vec<String>) -> Result<(), ExitCode> {
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&guard).expect("serialize ollama guard")
    );
