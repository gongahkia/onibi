...
export async function writeWebEvidenceFiles(
  outDir: string,
...
export async function readWebEvidenceBundle(path: string): Promise<WebEvidenceBundle> {
  const metadata = await stat(path);
  const evidencePath = metadata.isDirectory() ? join(path, "web-evidence.json") : path;
  return JSON.parse(await readFile(evidencePath, "utf8")) as WebEvidenceBundle;
}

export function webBom(bundle: WebEvidenceBundle) {
  return {
    schemaVersion: "1.0.0",
...
export function renderWebEvidenceHtml(bundle: WebEvidenceBundle): string {
...
<body>
  ...
  <h2>Request</h2>
  <pre>${escapeHtml(stableJsonStringify(bundle.request as unknown as JsonValue))}</pre>
  <h2>Sources</h2>
  <table>
  ...
  <h2>Events</h2>
  <pre>${escapeHtml(stableJsonStringify(bundle.events as unknown as JsonValue))}</pre>
</body>
</html>
`;
}
