...
describe("ApprovalInlineBanner", () => {
  ...
  test("submits edited Bash input as updatedInput", async () => {
    render(<ApprovalInlineBanner initialPending={pending} token="test-token" />);
  ...
  test("falls back to default deny reason when none supplied", async () => {
    render(<ApprovalInlineBanner initialPending={pending} token="test-token" />);
    fireEvent.click(screen.getByRole("button", { name: "Deny" }));
  ...
  test("requests informational attention once for a new realtime approval", async () => {
    ...
    try {
      render(<ApprovalInlineBanner token="test-token" />);
  ...
  test("clears queue and emits resolved toast on approval-resolved", async () => {
    globalThis.__TAURI_MOCKS__.listen.mockImplementation(async (_event, handler) => {
      handler({ payload: pending });
      return globalThis.__TAURI_MOCKS__.unlisten;
    });
    ...
    const onResolved = vi.fn();
    render(<ApprovalInlineBanner token="test-token" onResolved={onResolved} />);

    await waitFor(() =>
      expect(screen.getByRole("region", { name: "Approval request" })).toBeTruthy(),
    );

    const lastHandler = globalThis.__TAURI_MOCKS__.listen.mock.calls.at(-1)?.[1];
    ...
    expect(onResolved).toHaveBeenCalledWith(pending.approval_id);
    expect(screen.getByText(/Resolved on phone/)).toBeTruthy();
  });
});
