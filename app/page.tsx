...
function DesktopSheetRail({ activeSheet, entryCount, onAdd, onAllSheets }: { activeSheet?: Sheet; entryCount?: number; onAdd: () => void; onAllSheets?: () => void }) {
  return <aside className="desktop-sheet-rail" aria-label="Desktop workspace navigation"><div className="desktop-brand"><span><AppIcon name="receipt" size="md" /></span><div><b>Expenses</b><small>Personal finance</small></div></div><button type="button" className="desktop-rail-add" onClick={onAdd}><AppIcon name="plus" size="sm" />New item</button><p>Workspace</p>{onAllSheets ? <button type="button" className="desktop-rail-link" onClick={onAllSheets}><AppIcon name="table" size="sm" />All sheets</button> : <div className="desktop-rail-link current"><AppIcon name="table" size="sm" />All sheets</div>}{activeSheet && <div className="desktop-active-sheet"><span><AppIcon name="receipt" size="sm" /></span><div><b>{activeSheet.name}</b><small>{entryCount || 0} {entryCount === 1 ? "entry" : "entries"}</small></div></div>}<div className="desktop-rail-footer"><AppIcon name="lock" size="sm" /><span>Stored on this device</span></div></aside>;
}
...
function CategoryEditor({ category, categories, transactions, onCancel, onSave, onDelete }: { category: Category; categories: Category[]; transactions: Transaction[]; onCancel: () => void; onSave: (draft: Omit<Category, "id" | "sortOrder" | "updatedAt" | "deletedAt">) => void; onDelete: (replacement: string) => void }) {
  ...
  const [hexInput, setHexInput] = useState(category.color || categoryColors[0]);
  ...
  function chooseColor(next: string) { setColor(next); setHexInput(next.toLocaleUpperCase()); }
  function updateHex(next: string) { const normalized = next.startsWith("#") ? next : `#${next}`; setHexInput(normalized); if (/^#[0-9a-fA-F]{6}$/.test(normalized)) setColor(normalized.toLocaleUpperCase()); }
  return <form className="category-editor" onSubmit={(event) => { event.preventDefault(); onSave({ name, kind, icon, color }); }}>
    <div className="settings-tabs"><button type="button" className={kind === "expense" ? "selected" : ""} onClick={() => setKind("expense")}>Expense</button><button type="button" className={kind === "income" ? "selected" : ""} onClick={() => setKind("income")}>Income</button></div>
    <label className="category-name-input"><span className="category-picker-icon large" style={{ background: color }}><AppIcon name={icon} size="lg" /></span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Name your category" autoFocus /></label>
    <h2 className="settings-section-heading">Icon</h2>
    <div className="category-icon-grid">{categoryIcons.map((item) => <button type="button" className={icon === item ? "selected" : ""} onClick={() => setIcon(item)} key={item} aria-label={`Use ${item} icon`}><AppIcon name={item} size="md" /></button>)}</div>
    <h2 className="settings-section-heading">Color</h2>
    <div className="category-color-grid">{categoryColors.map((item) => <button type="button" className={color.toLocaleLowerCase() === item ? "selected" : ""} style={{ background: item }} onClick={() => chooseColor(item)} key={item} aria-label={`Use ${item} color`}>{color.toLocaleLowerCase() === item && <AppIcon name="check" size="sm" />}</button>)}</div>
    <section className="custom-color-card" aria-label="Custom category color"><label><span>Custom color</span><input type="color" value={/^#[0-9a-fA-F]{6}$/.test(color) ? color : categoryColors[0]} onChange={(event) => chooseColor(event.target.value)} aria-label="Pick a custom color" /></label><label><span>Hex</span><input value={hexInput} onChange={(event) => updateHex(event.target.value)} onBlur={() => { if (!/^#[0-9a-fA-F]{6}$/.test(hexInput)) setHexInput(color); }} inputMode="text" maxLength={7} spellCheck={false} aria-label="Custom color hex value" /></label></section>
    <button type="submit" className="settings-primary-button">Save Category</button>
    {category.id && <section className="category-delete-panel"><p>{used ? `${used} existing transaction${used === 1 ? "" : "s"} must move to another ${category.kind} category.` : "Move this category to Trash."}</p>{used && (alternatives.length ? <select value={replacement} onChange={(event) => setReplacement(event.target.value)}><option value="">Choose replacement</option>{alternatives.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select> : <p>Create another {category.kind} category before removing this one.</p>)}<button type="button" disabled={used > 0 && (!alternatives.length || !replacement)} className="settings-danger-button" onClick={() => onDelete(replacement)}>Move to Trash</button></section>}
