...
def test_deferred_adapter_expansion_keeps_host_work_parked() -> None:
    text = (ROOT / "docs" / "deferred-adapter-expansion.md").read_text(
        encoding="utf-8"
    )
...
def test_known_limitations_link_deferred_adapter_expansion() -> None:
    payload = json.loads(
        (ROOT / "docs" / "known-limitations.json").read_text(encoding="utf-8")
    )
    limitation = next(
        item for item in payload["limitations"] if item["id"] == "KL-001"
    )
