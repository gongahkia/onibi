...
with Diagram(
    ...
    with Cluster("macOS Menubar App", graph_attr={**cluster_attr, "bgcolor": "#1a1a3e"}):
        with Cluster("Views / UI Layer", graph_attr={**cluster_attr, "bgcolor": "#1e2749"}):
            menubar = Rack("MenuBarController")
            menuview = Rack("MenuBarView")
            settings_view = Rack("SettingsView")

        with Cluster("ViewModels", graph_attr={**cluster_attr, "bgcolor": "#1e2749"}):
            settings_vm = Rack("SettingsViewModel")
            notif_vm = Rack("NotificationListVM")

        with Cluster("Core Services", graph_attr={**cluster_attr, "bgcolor": "#162347"}):
        ...
        with Cluster("Log Processing Pipeline", graph_attr={**cluster_attr, "bgcolor": "#0d1b3e"}):
            file_watcher = Rack("FileWatcher\n(FSEvents)")
            log_buffer = Rack("LogBuffer")
            log_parser = Rack("LogFileParser")
            log_truncator = Rack("LogFile\nTruncator")

        with Cluster("Event Detection", graph_attr={**cluster_attr, "bgcolor": "#0d1b3e"}):
        ...
        with Cluster("Ghostty Integration", graph_attr={**cluster_attr, "bgcolor": "#162347"}):
            ipc_client = Rack("GhosttyIPC\nClient")
            cli_service = Rack("GhosttyCli\nService")
            process_mon = Rack("Process\nMonitor")
            shell_hooks = Rack("ShellHook\nInstaller")

        with Cluster("Data / Storage", graph_attr={**cluster_attr, "bgcolor": "#0d1b3e"}):
            json_storage = Storage("JSONStorage\nManager")
            lru_cache = Storage("LRU Cache")
            user_defaults = Storage("UserDefaults")

        with Cluster("Utilities", graph_attr={**cluster_attr, "bgcolor": "#0d1b3e"}):
            logger = Rack("Log\n(os.log)")
