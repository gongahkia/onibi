...
The publishable Rust API is `yeokcham-sdk`; its documented re-exports are the supported Rust surface. `yeokcham-core`, `yeokcham-protocol`, `yeokcham-daemon`, `yeokcham-daemon-api`, `yeokcham-relay`, `yeokcham-ffi`, and `yeokcham-cli` are internal Rust crates and set `publish = false`.

The C ABI is an external interface, but not a publishable Rust API. Its only compatibility identifier is `YEOKCHAM_ABI_VERSION`; callers must negotiate the exact value before invoking versioned ABI operations. C ABI ownership, threading, and callback rules are defined in `crates/yeokcham-ffi/README.md`.
...
`yeokcham-sdk` is currently `0.1.0`. Its public `SDK_API_VERSION` is the package major/minor pair and must be updated in the same release as the package version.
...
Every release that changes `yeokcham-sdk` must:

1. Update the package version and `SDK_API_VERSION` together.
2. Run `cargo test -p yeokcham-sdk --test api_version_contract --locked` to exercise the published Rust boundary.
3. Run `cargo test -p yeokcham-core --test workspace_policy --locked` to verify scope and policy enforcement.
