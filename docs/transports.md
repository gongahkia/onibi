...
ONIBI_MATRIX_OWNER_DEVICE_ID='OWNERDEVICE'
ONIBI_MATRIX_ALLOW_ENCRYPTED=0
ONIBI_MATRIX_SAS_VERIFIED=0
```

Onibi validates the configured room through Matrix Client-Server APIs, checks the bot user's room power level, refuses encrypted rooms by default, stores the room-scoped `/sync` `next_batch` token for reconnect, sends terminal output with `m.room.message`, and surfaces homeserver errors. Approval prompts are sent as room messages; react `✅` or `👍` to approve, react `❌` or `👎` to deny. Matrix text-in, tail chunks, approval sends, and reactions write `provider.matrix.*` audit rows.

Encrypted rooms require `ONIBI_MATRIX_ALLOW_ENCRYPTED=1`, an owner user/device pair, and an encrypted local store. On the first run, manually compare SAS on the owner device, then set `ONIBI_MATRIX_SAS_VERIFIED=1`; Onibi verifies and pins that device's signed Curve25519/Ed25519 keys in CryptBox. It shares outbound `m.room_key` only with that device, consumes owner `m.room_key` to-device events, decrypts its inbound Megolm text/reactions, and rejects repeated Megolm message indices. [Unverified] Onibi does not yet provide an interactive SAS verification UI, cross-signing, or a real encrypted-room integration result, so encrypted-room acceptance remains open.
