...
- `kelp-pi-validate-field-acceptance --output-dir <dir> ... --nuclei-approval-token <token>`
  ...
  Pi 5/aarch64 proof, aarch64 ELF proof for `/usr/local/bin/kelp-pi-agent`, exact
  `systemctl show` hardening properties, `systemd-analyze` score `<3.0`, and the
  `nft list ruleset` output chain showing default-drop plus established-session
  handling. Optional flags `--ollama-expect-load`, `--ollama-expect-refuse`, and
  `--readonly-root` add those evidence logs to the same artifact directory.
