...
The macOS, upgrade-recovery, and security-regression gates are release-blocking.
The latter two write `metadata.json`, `test.log`, and `summary.json` under
`artifacts/` for CI upload. Linux is beta-only; run
