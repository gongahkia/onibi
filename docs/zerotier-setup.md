...
- The laptop is authorized in a ZeroTier network and has a routed virtual IP before `onibi start` starts.
...
```bash
onibi system doctor --transport=zerotier
onibi start --transport=zerotier
```
...
Android, then restart `onibi start` so the generated server certificate includes
the active ZeroTier IP.

## Smoke

1. Run `onibi system doctor --transport=zerotier`.
2. Run `onibi start --transport=zerotier`.
...
While `onibi start` is running, it rechecks ZeroTier membership, interface, and selected address. If any changes, Onibi stops rather than minting or retaining a stale endpoint; restore the selected network and restart `onibi start`.
