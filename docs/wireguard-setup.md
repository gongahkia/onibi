...
- The WireGuard interface has a routable VPN IP before `onibi start` starts.
...
```bash
onibi system doctor --transport=wireguard
onibi start --transport=wireguard
```
...
1. Run `onibi system doctor --transport=wireguard`.
2. Run `onibi start --transport=wireguard`.
...
While `onibi start` waits, it rechecks the selected WireGuard interface and address every five seconds. If either changes or becomes unavailable, Onibi stops; restore the tunnel and restart `onibi start` to mint a new pairing URL.
