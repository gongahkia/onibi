...
The main challenge was keeping the project honest. A polished demo could hide the exact failure mode we wanted to show, so the fixture intentionally includes a baseline overclaim and hostile case-derived text.
...
The last challenge was keeping replay simple enough for judges. The current submission uses a deterministic offline trace in `fixtures/protocol-sift-baseline/` so the verifier, firewall, repair loop, spoliation check, and audit export can be rerun without requiring every judge to configure a live SIFT VM first.
