...
(A) 2026-06-29 Build-time gate in Makefile + .goreleaser.yaml: tagged releases (v* git tag) refuse to build if linker symbol search finds the string "unsafe-cloudflare-no-e2e" in the output binary; fail loudly +phaseQ0 @build file:Makefile id:T2028 accept:tagged-build-fails-when-gate-broken
