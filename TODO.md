...
(A) 2026-06-29 Add internal/store/cryptbox.go: wraps internal/envelope/envelope.go with row-scoped AAD (table_name||row_id||column_name); funcs Seal(ctx, plaintext, aad)([]byte, error) and Open(ctx, sealed, aad)([]byte, error); master key handle injected via NewStore constructor +phaseQ0 @backend file:internal/store/cryptbox.go id:T2000 accept:roundtrip-test-passes-with-random-aad
(A) 2026-06-29 Extend internal/secrets/ with GetOrCreateStoreKey(ctx)([]byte,error): reads onibi.store.key.v1 from OS keychain via zalando/go-keyring; if absent, generate 32 random bytes via crypto/rand.Read, write back, return; Linux fallback ~/.config/onibi/store.key with 0600 perms when secret-service unavailable +phaseQ0 @backend file:internal/secrets/ id:T2001 blocked-by:T2000 accept:key-survives-restart
(A) 2026-06-29 Schema migration 0007_encrypt_at_rest.sql: replace plaintext columns pairing_tokens.token, web_sessions.cookie, web_sessions.user_agent, audit_events.payload, plus chat-provider credential blobs in secrets table with <name>_enc BLOB columns; use existing migration mechanism in internal/store/migrations/ +phaseQ0 @backend file:internal/store/migrations/0007_encrypt_at_rest.sql id:T2002 blocked-by:T2001 accept:fresh-db-uses-only-_enc-columns
(A) 2026-06-29 Upgrade migration internal/store/migrations/0007_upgrade.go: read each plaintext row, call cryptbox.Seal, write <name>_enc column, drop plaintext column on commit; idempotent — skip rows already converted +phaseQ0 @backend file:internal/store/migrations/ id:T2003 blocked-by:T2002 accept:upgrade-from-v0.3-db-succeeds
(A) 2026-06-29 Update every accessor in internal/store/sqlite.go to Seal on write and Open on read through cryptbox; audit-event logging must keep working during migration; race tests stay green +phaseQ0 @backend file:internal/store/sqlite.go id:T2004 blocked-by:T2003 accept:race-tests-still-green
(B) 2026-06-29 Add onibi store rekey CLI subcommand: generate fresh master key, re-Seal every encrypted column under it, rotate keychain entry; useful after a suspected compromise +phaseQ0 @backend file:internal/cli/store.go id:T2005 blocked-by:T2004 accept:rekey-produces-decryptable-db
(B) 2026-06-29 internal/store/cryptbox_test.go: round-trip, AAD-mismatch rejection, tampered-ciphertext rejection, empty-plaintext handling; -race -count=1 must be green +phaseQ0 @tests file:internal/store/cryptbox_test.go id:T2006 blocked-by:T2000 accept:race-tests-cover-tamper-cases
(B) 2026-06-29 Extend internal/doctor/ to verify keychain entry present + decryptable; fail loudly with remediation steps if absent +phaseQ0 @backend file:internal/doctor/ id:T2007 blocked-by:T2005 accept:doctor-flags-missing-key
...
(A) 2026-06-29 Write docs/SPEC-e2e.md: full protocol — URL fragment shape #k=<43char-b64url-encoded-32B>, KDF (HKDF-SHA256 with salt=session_id, info="onibi-e2e-v1"), framing (12B IV + ciphertext + 16B GCM tag), AAD shape, verifier flow, sequence-number replay defense per direction, threat model + non-goals (relay can drop/reorder/correlate metadata but cannot read) +phaseQ0 @docs file:docs/SPEC-e2e.md id:T2020 blocked-by:T2008 accept:spec-reviewed-and-frozen
(A) 2026-06-29 Mark existing TODO Future-tasks T1300/T1301/T1302/T1303 as superseded by T2020–T2031; edit inline `x 2026-06-29 superseded by T2020-T2031` +phaseQ0 @docs file:TODO.md id:T2021 blocked-by:T2020 accept:old-tasks-marked-superseded
(A) 2026-06-29 Add internal/e2e/envelope.go with DeriveSessionKey(masterKey, sessionID []byte) []byte returning 32-byte AES-GCM key via HKDF-SHA256; reuse primitives from internal/envelope/envelope.go +phaseQ0 @backend file:internal/e2e/envelope.go id:T2022 blocked-by:T2021 accept:matches-webcrypto-test-vector
(A) 2026-06-29 Go test vectors for HKDF-then-AES-GCM round-trip; vectors importable into frontend/src/e2e.test.ts so the same key+IV+AAD+plaintext produces identical ciphertext under WebCrypto subtle +phaseQ0 @tests file:internal/e2e/envelope_test.go id:T2023 blocked-by:T2022 accept:cross-impl-vectors-match
(A) 2026-06-29 Implement E2E framing in internal/web/ws_pty.go: when session.E2E.Enabled, outbound writes are Seal(plaintext, aad) and inbound reads are Open(ciphertext, aad); per-direction sequence numbers monotonically increase; reject on AAD or seq violation with WS close 1008 +phaseQ0 @backend file:internal/web/ws_pty.go id:T2024 blocked-by:T2023 accept:seq-replay-rejected
(A) 2026-06-29 Server stores key_verifier = HKDF(K, salt=session_id, info="onibi-verifier-v1", L=32) in web_sessions.key_verifier_enc (encrypted via Q0a); on handshake client sends verify_token = HKDF(K, salt=session_id, info="onibi-verify-token-v1", L=32); server constant-time compares against stored verifier; server CANNOT decrypt traffic by construction +phaseQ0 @backend file:internal/web/auth.go id:T2025 blocked-by:T2024 accept:server-cannot-decrypt-payload
(A) 2026-06-29 Extend frontend/src/e2e.ts: parse K from location.hash.slice(1).match(/k=([A-Za-z0-9_-]+)/); decode base64url to 32B; derive session key via subtle.deriveKey HKDF-SHA256; wrap WS frames in encrypt/decrypt per spec; never log K, never POST K to server, never echo K to console +phaseQ0 @frontend file:frontend/src/e2e.ts id:T2026 blocked-by:T2025 accept:no-key-in-devtools-network-tab
(A) 2026-06-29 Update internal/web/transport/cloudflare.go: when transport=cloudflare-quick, force E2E enabled; QR encodes https://<id>.trycloudflare.com/#k=<base64url(k)> with K never sent to relay +phaseQ0 @backend file:internal/web/transport/cloudflare.go id:T2027 blocked-by:T2026 accept:QR-includes-fragment
(A) 2026-06-29 Build-time gate in Makefile + .goreleaser.yaml: tagged releases (v* git tag) refuse to build if linker symbol search finds the string "unsafe-cloudflare-no-e2e" in the output binary; fail loudly +phaseQ0 @build file:Makefile id:T2028 blocked-by:T2027 accept:tagged-build-fails-when-gate-broken
(B) 2026-06-29 Update /healthz to return {"e2e": <bool>, "key_verifier_hex": "<32hex>"} when E2E session active; phone can compute the expected verifier locally and warn on mismatch (defense vs. server lying about being E2E) +phaseQ0 @backend file:internal/web/server.go id:T2029 blocked-by:T2028 accept:health-payload-includes-verifier
(B) 2026-06-29 Verify WS app-level ping at 30s (TODO §3 rule 10) still fires after E2E framing wraps it; CF 100s idle must not disconnect; test by holding an idle WS open for 4 minutes +phaseQ0 @backend file:internal/web/ws_pty.go id:T2030 blocked-by:T2029 accept:no-cf-idle-disconnect-in-4min-test
(B) 2026-06-29 Real-iPhone E2E smoke: onibi up --transport=cloudflare-quick; iPhone on LTE scans QR with fragment; drives PTY; tcpdump on a third box capturing the trycloudflare.com side shows only ciphertext (no vim/shell strings visible) +phaseQ0 @tests id:T2031 blocked-by:T2030 accept:tcpdump-shows-no-plaintext

#### Q0c — F: Plugin / adapter SDK + SPEC.md

(A) 2026-06-29 Write docs/SPEC-adapters.md: TOML adapter manifest schema; required keys name, version, kind ("agent"|"shell"), cmd_pattern (glob match on PreToolUse tool name → action), hook_install (shell commands run at install-hooks time, restricted), hook_uninstall, risk_overrides (per-tool risk level adjustments), min_onibi_version; appendix shows the 8 in-tree adapters as reference implementations +phaseQ0 @docs file:docs/SPEC-adapters.md id:T2060 blocked-by:T2031 accept:spec-frozen-and-versioned
(A) 2026-06-29 Refactor internal/adapters/ into a Registry interface: Register(manifest Manifest) error, List() []Manifest, Get(name) (Manifest,error); existing 8 in-tree adapters become loaders that register themselves via init() +phaseQ0 @backend file:internal/adapters/registry.go id:T2061 blocked-by:T2060 accept:existing-adapter-tests-still-pass
(A) 2026-06-29 onibi adapters add <path> subcommand: read local TOML, validate against SPEC, copy to ~/.config/onibi/adapters/<name>.toml, register; URL form (onibi adapters add https://…) fetches with crypto/tls-verified TLS and requires --sha256=<hex> pinning flag +phaseQ0 @backend file:internal/cli/adapters.go id:T2062 blocked-by:T2061 accept:third-party-toml-loads-and-routes-hooks
...
(A) 2026-06-29 Extend internal/daemon/approvals.go: when PreToolUse event tool name is in {Edit, MultiEdit, Write, NotebookEdit}, compute unified diff (call out to `diff -u oldfile newfile` via os/exec, or use a Go library); apply internal/approval/scrub.go to both sides BEFORE diffing so redacted strings stay redacted in the diff; attach unified_diff field to ApprovalEvent JSON payload +phaseQ1 @backend file:internal/daemon/approvals.go id:T2100 blocked-by:T2066 accept:edit-events-include-redacted-diff
(A) 2026-06-29 Add diff2html@^3.4.56 to frontend/package.json; lazy import via dynamic import() inside approval.ts so non-Edit approvals do not pay the 25KB bundle cost +phaseQ1 @frontend file:frontend/package.json id:T2101 blocked-by:T2100 accept:dynamic-import-confirmed-in-build-output
(A) 2026-06-29 Extend frontend/src/approval.ts: when event has unified_diff, render via diff2html Diff2HtmlUI(elem, diff, {drawFileList:false, matching:"lines", outputFormat:"line-by-line"}); mobile-portrait forces line-by-line; landscape uses side-by-side; preserve existing Approve/Deny/Edit buttons +phaseQ1 @frontend file:frontend/src/approval.ts id:T2102 blocked-by:T2101 accept:diff-renders-on-iphone-portrait
