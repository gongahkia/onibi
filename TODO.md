...
(A) 2026-06-23 Create frontend/ directory with package.json declaring devDependencies: vite ^5, typescript ^5, @types/node; dependencies: xterm ^5, @xterm/addon-fit ^0.10, @xterm/addon-attach ^0.11 +phase04 @frontend file:frontend/package.json id:T400 blocked-by:T215
(A) 2026-06-23 Add frontend/tsconfig.json: strict true, target ES2020, module ESNext, moduleResolution Bundler, lib ["ES2020","DOM"] +phase04 @frontend file:frontend/tsconfig.json id:T401 blocked-by:T400
(A) 2026-06-23 Add frontend/vite.config.ts: outDir "../internal/web/static/dist", emptyOutDir true, build.assetsDir "assets" with content-hashed filenames (vite default), build.manifest true so server can map entry to hashed name +phase04 @frontend file:frontend/vite.config.ts id:T402 blocked-by:T401 ref:arch-rule-canvas-renderer
(A) 2026-06-23 Add frontend/index.html: <!doctype html>, viewport meta with viewport-fit=cover and user-scalable=no, html/body height: 100dvh, dark background, mount point div#term +phase04 @frontend file:frontend/index.html id:T403 blocked-by:T402 ref:arch-rule-2
(A) 2026-06-23 Write frontend/src/terminal.ts: createTerminal() returns Terminal with rendererType "canvas" (FORCE canvas, NOT WebGL), font "Menlo,Monaco,monospace", fontSize 14, cursorBlink true; load FitAddon and call fit() +phase04 @frontend file:frontend/src/terminal.ts id:T404 blocked-by:T403 ref:arch-rule-8
(A) 2026-06-23 Write frontend/src/ws.ts: class TerminalWS with connect(url, sessionId, lastSeq) → opens WS with binaryType="arraybuffer", subprotocols ["onibi.pty.v1"]; emits 'data' (ArrayBuffer), 'resize' (rows,cols), 'close' events; auto-reconnect with exponential backoff capped at 8s +phase04 @frontend file:frontend/src/ws.ts id:T405 blocked-by:T404
(A) 2026-06-23 Implement resume protocol on TerminalWS.connect: send opening JSON frame {type:"attach", session_id, last_seq} as first text frame; server may respond with {type:"snapshot", seq, base64_data} which client decodes and writes to term before resuming live binary stream +phase04 @frontend file:frontend/src/ws.ts id:T406 blocked-by:T405
(A) 2026-06-23 Wire term.onData (text input) → ws.send as text frame; wire term.onBinary → ws.send as binary frame; document that we deliberately use mixed text+binary frames and the server must distinguish +phase04 @frontend file:frontend/src/terminal.ts id:T407 blocked-by:T406
(A) 2026-06-23 Add visualViewport listener: on viewport resize, call FitAddon.fit() then ws.send JSON {type:"resize", rows, cols}; debounce 200ms +phase04 @frontend file:frontend/src/terminal.ts id:T408 blocked-by:T407 ref:arch-rule-2
(A) 2026-06-23 Server-side: implement /ws/pty protocol handling — read initial JSON attach frame, look up *pty.Host via session_id, call Host.Subscribe; pump subscriber bytes → WS binary frames; pump incoming WS text/binary frames → Host.Write; on resize JSON call Host.Resize +phase04 @backend file:internal/web/ws_pty.go id:T409 blocked-by:T408
(A) 2026-06-23 Server-side: on resume request with last_seq, if requested seq still in ring buffer send delta; else send snapshot frame {type:"snapshot", seq, base64_data} via JSON text frame then resume binary +phase04 @backend file:internal/web/ws_pty.go id:T410 blocked-by:T409
(A) 2026-06-23 Add frontend/src/main.ts entry: read sessionId from /session-info API (returns first/active session if exactly one) or from query param; instantiate terminal, ws; mount #term +phase04 @frontend file:frontend/src/main.ts id:T411 blocked-by:T410
(A) 2026-06-23 Embed bundle: add internal/web/static/embed.go with go:embed directive for static/dist/**; server serves /assets/* from embed with Cache-Control: public, max-age=31536000, immutable; root / serves index.html (not cached) +phase04 @backend file:internal/web/static/embed.go id:T412 blocked-by:T411
(A) 2026-06-23 Add Makefile targets: frontend-install (cd frontend && npm install), frontend-build (cd frontend && npm run build), make build depends on frontend-build before go build; document in Makefile comments +phase04 @build file:Makefile id:T413 blocked-by:T412
(B) 2026-06-23 CI: bundle-size gate — fail if total size of internal/web/static/dist/assets/*.js gz > 250 KB +phase04 @build file:Makefile id:T414 blocked-by:T413
...
(B) 2026-06-23 Unit tests on the WS server: attach with valid last_seq returns delta; attach with stale last_seq returns snapshot frame; resize frame triggers Host.Resize +phase04 @tests file:internal/web/ws_pty_test.go id:T416 blocked-by:T410
(C) 2026-06-23 Add a "loading…" splash in index.html that disappears once terminal is attached +phase04 @frontend file:frontend/index.html id:T417 blocked-by:T411
