...
(A) 2026-06-29 Verify chain: download checksums.txt + checksums.txt.sig + selected archive; verify signature via embedded GPG public key (build-time constant from .goreleaser GPG fingerprint); verify SHA-256 of archive; extract binary; on macOS additionally run `codesign --verify` on extracted binary; only then call selfupdate.Apply(binary, selfupdate.Options{}) +phaseQ4 @backend file:internal/cli/update.go id:T2461 accept:checksum-mismatch-aborts
