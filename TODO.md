...
(A) 2026-06-29 Go test vectors for HKDF-then-AES-GCM round-trip; vectors importable into frontend/src/e2e.test.ts so the same key+IV+AAD+plaintext produces identical ciphertext under WebCrypto subtle +phaseQ0 @tests file:internal/e2e/envelope_test.go id:T2023 accept:cross-impl-vectors-match
