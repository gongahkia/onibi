...
### Implemented in the code-scope hardening pass

- Removed Git clone, push, and pull surfaces from the active desktop UI/backend bridge while keeping status, diff, staging, and commit.
...
| **Remote attach** | Yes — SSH bootstrap, auto-install on remote | Partial — SSH-backed local PTY remote sessions with metadata, CLI/API/dialog launch, remote keybinding policy, helper bootstrap, remote helper daemon start, and remote daemon session stream attach; no existing-PTY transfer yet |
...
| **Git status / staging / commit UI** | Status caching, sidebar indicators | Focused UI (status, staging, commit, diffs; clone/push/pull cut) |
...
### 2.6 Remote
34. **[DONE] SSH remote attach** (`herdr --remote ssh://…`) — SSH-backed local PTY sessions are implemented through GUI, CLI, desktop API, remote metadata persistence, keybinding policy, current-binary helper bootstrap, remote helper daemon start, and remote daemon session stream attach. Existing-live-PTY transfer/handoff remains a separate future capability.
...
- Git UI: clone / push / pull are cut and removed from active UI/backend surfaces. Source Control stays focused on status, diff, staging, and commit.
...
119. **[DONE] Auto-worktree per session (optional).** Extend the existing worktree CLI / Explorer surface (item 71/72) so launching an agent session can optionally create and bind a fresh worktree, isolating that session's filesystem mutations from the parent workspace. Useful in combination with item 118 (checkpoints) and the trust-mode toggle (item 123).
    ...
    **Mapping to onibi:** add `worktreeStrategy: "inherit" | "auto"` to session-start metadata; `auto` creates `<repo>/.onibi-worktrees/<id>/` before launch, binds the session to that workspace, and removes it best-effort on session close if clean. Surface as a checkbox in the New Session dialog and as `--worktree auto` on `onibi session launch`.
...
122. **[DONE] Lexical rich-text composer for prose inputs only.** Replace the plain-`<textarea>` deny-reason field, approval edit-input field (for non-Bash, non-code payloads), and agent-prompt composer with a Lexical-backed editor. **Keep CodeMirror for Bash / `Write` / `Edit` / `MultiEdit` previews and edit fields** — that scope remains correct.
    ...
    **Mapping to onibi:** new `app/src/components/composer/ProseComposer.tsx` wrapping `@lexical/react`; replace prose fields in `ApprovalModal.tsx` and `NewSessionDialog.tsx`. Keep payload-shape backward-compatible: Lexical serialises to plain text before submission.
