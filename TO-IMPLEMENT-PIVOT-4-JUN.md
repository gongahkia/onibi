...
3. **[PARTIAL] Drag-reorder for workspaces / tabs / panes** — workspace reorder and workspace terminal-tab reorder are done; pane reorder remains.
...
6. **[PARTIAL] Pane tiling layout math** — split tree and split sizes persist per workspace tab; deeper tmux-style layout commands remain.
...
68. **[PARTIAL] `new_cwd` policy** — implemented `active`, `workspace`, and `home`; fixed paths and `follow` remain.
...
Phase A deepening is now mostly complete. Remaining foundation work is pane reorder, deeper tmux-style layout commands, indexed bindings, custom command keybindings, config reset/reload, and fixed-path/follow CWD policy.

Completed or mostly completed from original Phase A:
1, 2, 4, 5, 7, 9, 21, 22, 23, 24, 48, 49, 50, 53, 64, 65.
...
3 (pane reorder only), 37 (decision: keep xterm.js or embed ghostty-vt), 38, 40, 43, 44, 45, 55, 56.

**Phase D — remote & distribution:**
34, 35, 62, 73, 74.

**Phase E — long tail:**
46, 51, 52, 54, 58, 59, 60, 63, 66, 67, 68, 75, 76, 78–83.
