.PHONY: build frontend-install frontend-build frontend-size-check remote-static-archive install test vet staticcheck tidy run clean gen-readme gen-readme-check docs-pages docs-pages-check fresh-machine-doc-check linux-beta-smoke macos-release-gate macos-scenario-gate security-regression-gate release-e2e-gate release-dry release-smoke reproducible-build bench-tolerance install-pages
...
docs-pages:
	go run ./scripts/render-docs

docs-pages-check:
	go run ./scripts/render-docs --check
