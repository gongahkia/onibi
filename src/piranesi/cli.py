...
@ingest_app.command("nessus", help="Ingest Nessus .nessus XML into a workspace.")
def ingest_nessus_command(
    input_path: Annotated[
        Path,
        typer.Option(
            "--input",
            ...
            dir_okay=False,
            file_okay=True,
            help="Nessus .nessus XML export to ingest.",
        ),
    ],
    workspace: Annotated[
        Path,
    ...
    try:
        summary, parse_result = _ingest_findings_export(
            ...
            workspace=workspace,
            tool="nessus",
            command="ingest nessus",
            parser=lambda path, digest, raw_path: parse_nessus_file(
                path,
                input_sha256=digest,
                raw_path=raw_path,
            ),
        )
    except (WorkspaceError, NessusParseError) as exc:
        _fail(str(exc), json_errors=json_errors)
...
@ingest_app.command("sarif", help="Ingest SARIF 2.1.0 findings into a workspace.")
def ingest_sarif_command(
    input_path: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            file_okay=True,
            help="SARIF 2.1.0 JSON export to ingest.",
        ),
    ],
    workspace: Annotated[
        Path,
    ...
    try:
        summary, parse_result = _ingest_findings_export(
            ...
            parser=lambda path, digest, raw_path: parse_sarif_file(
                path,
                input_sha256=digest,
                raw_path=raw_path,
            ),
        )
    except (WorkspaceError, SarifParseError) as exc:
        _fail(str(exc), json_errors=json_errors)
...
@ingest_app.command("sqlmap", help="Ingest sqlmap JSON or log artifacts into a workspace.")
def ingest_sqlmap_command(
    input_path: Annotated[
        Path,
        typer.Option(
            "--input",
            ...
            dir_okay=False,
            file_okay=True,
            help="sqlmap JSON or text output artifact to ingest.",
        ),
    ],
    workspace: Annotated[
        Path,
    ...
    try:
        summary, parse_result = _ingest_findings_export(
            ...
            workspace=workspace,
            tool="sqlmap",
            command="ingest sqlmap",
            parser=lambda path, digest, raw_path: parse_sqlmap_file(
                path,
                input_sha256=digest,
                raw_path=raw_path,
            ),
        )
    except (WorkspaceError, SqlmapParseError) as exc:
        _fail(str(exc), json_errors=json_errors)
