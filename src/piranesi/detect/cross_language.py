...
def extract_api_boundaries(project_root: str | Path) -> list[ApiBoundary]:
    ...
    for path in sorted(root.rglob("*")):
        if not path.is_file():
            continue
        if any(part.startswith(".") or part in ("node_modules", "venv", ".venv", "__pycache__", "vendor", "target", "build", "dist") for part in path.parts):
            continue
...
def _extract_line_boundaries(
    ...
    if lang == "typescript":
        for pattern in (_FETCH_PATTERN, _AXIOS_PATTERN, _HTTP_CLIENT_PATTERN):
            for m in pattern.finditer(line):
                out.append(ApiBoundary(
                    url_path=m.group(1), file=rel_path, line=line_no,
                    snippet=line.strip(), language=lang, direction="client",
                ))
    ...
    if lang == "python":
        for m in _FLASK_ROUTE_PATTERN.finditer(line):
            out.append(ApiBoundary(
                url_path=m.group(1), file=rel_path, line=line_no,
                snippet=line.strip(), language=lang, direction="server",
            ))
...
def _find_taint_in_context(
    ...
    *,
    ...
    pattern = _PYTHON_SINK_PATTERNS if lang == "python" else _GO_SINK_PATTERNS if lang == "go" else _JAVA_SINK_PATTERNS
...
def _classify_sink(sink_snippet: str) -> tuple[str, str]:
    ...
    if "execute" in s and ("cursor" in s or "query" in s or "jdbc" in s or "statement" in s or "db." in s):
        return "sql_injection", "CWE-89"
    if any(k in s for k in ("os.system", "subprocess", "exec.command", "processbuilder", "runtime")):
...
def detect_cross_language_flows(
    ...
    for client, server in matched:
        ...
        flows.append(CrossLanguageFlow(
            ...
            vuln_class=vuln_class, cwe_id=cwe_id,
        ))
    return flows
    ...
    for flow in flows:
        ...
        source = TaintSource(
            location=SourceLocation(
                file=flow.client.file, line=flow.client.line, column=0,
        ...
        sink = TaintSink(
            location=SourceLocation(
                file=flow.server.file, line=flow.server.line, column=0,
        ...
        api_step = TaintStep(
            location=SourceLocation(
                file=flow.client.file, line=flow.client.line, column=0,
                snippet=f"cross-language API call: {flow.client.language} → {flow.server.language} via {flow.client.url_path}",
            ),
