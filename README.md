...
- Experimental notify-only approvals: Pushover, ntfy, Gotify, APNs, and SMS publish approval alerts without terminal text input after the same explicit opt-in.
...
- `internal/pushover`, `internal/ntfy`, `internal/gotify`, `internal/apns`, and `internal/sms` provide notify-only approval delivery.
