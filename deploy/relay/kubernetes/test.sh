...
namespace=arachne-relay
client_namespace=arachne-relay-client
image=ghcr.io/gongahkia/arachne-relay:0.1.0
kind_cluster=arachne-relay-manifest-test-$$
temporary_directory=$(mktemp -d "${TMPDIR:-/tmp}/arachne-relay-kubernetes.XXXXXX")
...
kubectl -n "$namespace" create secret generic arachne-relay-identity --from-file=relay.identity="$identity_directory/relay.identity" >/dev/null
kubectl -n "$namespace" wait --for=condition=Ready "pod/$pod" --timeout=180s
kubectl -n "$namespace" exec "$pod" -- /usr/local/bin/arachne-relay healthcheck --address 127.0.0.1:8080 >/dev/null

service_ip=$(kubectl -n "$namespace" get service arachne-relay --output=jsonpath='{.spec.clusterIP}')
...
kubectl label namespace "$client_namespace" arachne.io/relay-client=true >/dev/null
