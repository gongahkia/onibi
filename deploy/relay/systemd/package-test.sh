...
temporary_directory=$(mktemp -d "${TMPDIR:-/tmp}/arachne-systemd-package-test.XXXXXX")
cleanup() {
    if [ -n "${relay_pid:-}" ]; then
...
package="$temporary_directory/arachne-relay.deb"
"$package_script" --output "$package"
[ -s "$package" ] || die 'package was not created'
[ "$(dpkg-deb -f "$package" Package)" = arachne-relay ] || die 'package name is invalid'
...
[ -x "$package_root/usr/bin/arachne-relay" ] || die 'relay binary is missing'
cmp "$package_root/etc/arachne/relay.conf" "$script_directory/../relay.conf.example" >/dev/null || die 'relay config is invalid'
cmp "$package_root/lib/systemd/system/arachne-relay.service" "$script_directory/arachne-relay.service" >/dev/null || die 'systemd unit is invalid'
[ -d /lib/systemd/system ] || die '/lib/systemd/system is required'
cp -a /lib/systemd/system/. "$package_root/lib/systemd/system/"
systemd-analyze verify --root="$package_root" arachne-relay.service >/dev/null

identity="$temporary_directory/relay.identity"
relay_binary="$package_root/usr/bin/arachne-relay"
...
sed "s|^listen_address = .*|listen_address = \"127.0.0.1:$relay_port\"|;s|^database_path = .*|database_path = \"$temporary_directory/relay.sqlite\"|" "$package_root/etc/arachne/relay.conf" > "$runtime_config"
