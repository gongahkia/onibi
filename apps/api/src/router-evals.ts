...
] as const satisfies readonly WorkflowRouterEvalCase[];

export const routerEvalCases: readonly WorkflowRouterEvalCase[] = baseRouterEvalCases.map(
  (testCase) => ({
    ...testCase,
    expectedNodeKinds:
      (testCase as WorkflowRouterEvalCase).expectedNodeKinds ??
      expectedNodeKindsForRoute(testCase.expectedRoute)
  })
);

export function runRouterEvalCases(
  input: {
...
export function routerEvalSummary(run: WorkflowRouterEvalRun): string {
...
export function routeKindFromString(value: string): WorkflowTaskRouteKind | undefined {
  return ["deterministic", "adapter", "codegen", "agentic", "deployment"].includes(value)
    ? (value as WorkflowTaskRouteKind)
    : undefined;
}
