...
final class FalsePositiveReducer: ObservableObject {
    ...
    private func loadDismissalCounts() {
        if let data = UserDefaults.standard.data(forKey: dismissalCountsKey),
           let decoded = try? JSONDecoder().decode([String: Int].self, from: data) {
            // Convert string keys back to NotificationType
    ...
    private func saveDismissalCounts() {
        // Convert NotificationType keys to strings for JSON encoding
        let stringDict = dismissalCounts.reduce(into: [:]) { result, pair in
            result[pair.key.rawValue] = pair.value
        }
        if let encoded = try? JSONEncoder().encode(stringDict) {
            UserDefaults.standard.set(encoded, forKey: dismissalCountsKey)
        }
    }
