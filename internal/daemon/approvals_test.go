...
func TestCodexApprovalDenyIsEnforcingAndAudited(t *testing.T) {
	db := openDaemonTestDB(t)
	d := New(Options{DB: db})
	if err := d.Registry.Add(NewSession("s1", "codex", "codex", nil, 0)); err != nil {
		t.Fatal(err)
	}
	events, unsubscribe, err := d.Queue.Subscribe()
	if err != nil {
...
func TestPiApprovalDenyIsEnforcingAndAudited(t *testing.T) {
	db := openDaemonTestDB(t)
	d := New(Options{DB: db})
	if err := d.Registry.Add(NewSession("s1", "pi", "pi", nil, 0)); err != nil {
		t.Fatal(err)
	}
	events, unsubscribe, err := d.Queue.Subscribe()
	if err != nil {
