...
func TestWorkspaceAddUseListRemove(t *testing.T) {
	...
	out, _ := executeRoot(t, "workspace", "add", "alpha", repo, "--default-transport", "tailscale", "--use", "--color", "never")
	...
	out, _ = executeRoot(t, "workspace", "remove", "alpha", "--color", "never")
...
func TestWorkspaceExportWritesPortableBundle(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(onibiDir, "budget.toml"), []byte("[session]\nmax_tokens = 25000\non_overrun = \"interrupt\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	executeRoot(t, "workspace", "add", "alpha", repo, "--ssh-key", "secret-key-ref", "--default-transport", "tailscale", "--color", "never")
	bundle := filepath.Join(t.TempDir(), "bundle")
	out, _ := executeRoot(t, "workspace", "export", "alpha", bundle, "--color", "never")
	...
	if _, err := os.Stat(filepath.Join(bundle, ".onibi", "budget.toml")); err != nil {
		t.Fatal(err)
	}
}
...
func TestWorkspaceImportBundleUse(t *testing.T) {
	...
	out, _ := executeRoot(t, "workspace", "import", repo, "--use", "--color", "never")
