...
func TestProfileCommandsAddListUseRemove(t *testing.T) {
	withDefaultState(t)
	cwd := t.TempDir()
	out, _ := executeRoot(t, "profile", "add", "work", "--transport", "tailscale-private", "--agent", "sh", "--cwd", cwd, "--color", "never")
	...
	for _, want := range []string{"work", "transport=tailscale-private", "agent=sh", "cwd=" + cwd} {
