...
func runRun(cmd *cobra.Command, args []string) error {
	...
	if intake.SocketActive(paths.Socket, 200*time.Millisecond) {
		return fmt.Errorf("onibi service is already running on %s; start sessions from Telegram with /new shell or /new <agent>, or stop the service before using foreground onibi run/shell", paths.Socket)
	}
