...
func promptWebTransport(cmd *cobra.Command, sc *bufio.Scanner, current string, style cliStyle) (string, error) {
	...
	for {
		...
		fmt.Fprintln(cmd.OutOrStdout(), "Choose 1, 2, 3, 4, 5, 6, 7, 8, or q.")
	}
}
...
func pairTransportChoices(current string) []pairTransportChoice {
	return []pairTransportChoice{
		...
		{key: "6", mode: "cloudflare-named", label: "Cloudflare Named", detail: "configured public hostname", coverage: "unit + fake process + live opt-in", command: "onibi up --transport=cloudflare-named", active: current == "cloudflare-named"},
		...
		{key: "8", mode: "auto", label: "Auto", detail: "try Tailscale, fallback LAN", coverage: "Tailscale -> LAN only", command: "onibi up --transport=auto", active: current == "auto"},
	}
}
