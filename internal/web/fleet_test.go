...
func TestFleetStatusAggregatesOwnerHomeReadModel(t *testing.T) {
	...
	srv.sessionList = func(_ context.Context, opts SessionListOptions) ([]SessionSummary, error) {
		...
		return []SessionSummary{{ID: "session-home", HostID: host.ID, Agent: "claude", LastActivity: now.Format(time.RFC3339Nano), PendingApprovalsCount: 1, RecoveryState: fleet.SessionRecoveryOrphaned, RecoveryReason: "tmux reconnect timed out", RecoveryUpdatedAt: now.Format(time.RFC3339Nano), RoleRequired: "owner", Remote: true, RemoteURL: "https://work.tail.ts.net/"}}, nil
	}
