...
func TestProjectConfigRejectsRemovedNtfyTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"ntfy\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestProjectConfigRejectsRemovedPushoverTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"pushover\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestProjectConfigRejectsRemovedSignalTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"signal\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestProjectConfigRejectsRemovedIRCTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"irc\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestProjectConfigRejectsRemovedZulipTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"zulip\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestProjectConfigRejectsRemovedDiscordTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"discord\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestProjectConfigRejectsRemovedMatrixTransport(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workspace.toml")
	if err := os.WriteFile(path, []byte("schema_version = 1\nname = \"alpha\"\n[transports]\ndefault = \"matrix\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
