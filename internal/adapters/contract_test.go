...
func TestCertifiedContractsMatchV1AgentAllowlist(t *testing.T) {
	...
	if c, ok := ContractFor("copilot"); !ok || c.Certified || c.Version != CertifiedContractVersion || c.MinimumProviderVersion != "1.0.54" || !c.Installation.Idempotent || !c.Approval.BlocksTool || c.Approval.Edit != DecisionAllowWithUpdated || c.Approval.RequestTimeout != DecisionAllow || c.Audit.DecisionRecorded {
		t.Fatalf("Copilot capability report=%+v present=%t", c, ok)
	}
