...
func TestPluginInstallIsIdempotentAndUninstallKeepsOriginalBackup(t *testing.T) {
	...
	notify := filepath.Join(dir, "onibi-notify")
	t.Setenv("ONIBI_AMP_PLUGIN", path)
	if err := os.WriteFile(path, []byte("export default function original() {}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(notify, []byte("#!/bin/sh\nexit 0\n"), 0o755); err != nil {
		t.Fatal(err)
	...
	info := Status(ctx, db)
	if !info.Installed || !info.Managed || info.Tampered {
	...
	if err != nil || string(body) != "export default function original() {}\n" {
